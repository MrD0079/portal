...
if (isset($_REQUEST["add"]))
{
	...
	if (isset($_REQUEST["bonus"]))
	{
		foreach ($_REQUEST["bonus"] as $key => $val)
		{
				...
				$values = array('bonus'=>str_replace(",", ".", $val));
			Table_Update ($table_name, $keys, $values);
		}
	}
...
$sql = rtrim(file_get_contents('sql/hot_july_process_1_nakl.sql'));
$sql=stritr($sql,$params);
//echo $sql;
...
{
$fn=$db->getOne("select fn from hot_july_files where id=".$k);
unlink("hot_july_files/".$tn."/1/".$fn);
$fn=$db->query("delete from hot_july_files where id=".$k);
}
}
...
if ($_FILES["new_fn"]["name"]!="")
{
	...
	if ($_FILES["new_fn"]['error']==0){
		...
		Table_Update("hot_july_files",$_REQUEST["new"],$_REQUEST["new"]);
	}
}
}
...
$sql=rtrim(file_get_contents('sql/hot_july_files.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/hot_july_process_1_tp.sql'));
$sql=stritr($sql,$params);
...
$params=array(':dpt_id' => $_SESSION["dpt_id"],':tn'=>$tn,':tp'=>0,':pt'=>$_REQUEST["pt"],':eta_list' => "'".$_REQUEST["eta_list"]."'");
$sql=rtrim(file_get_contents('sql/hot_july_process_1_nakl_total.sql'));
$sql=stritr($sql,$params);
$sql=stritr($sql,$params);
...
$smarty->display('hot_july_process_1.html');
?>