...
else
{
	if (isset($_REQUEST["head"]))
	{
		...
		{
			$x = $db->getOne("SELECT id FROM bud_ru_cash_out_head WHERE fil = ".$_REQUEST["head"]["fil"]." AND dt = TO_DATE ('".$_REQUEST["head"]["dt"]."', 'dd.mm.yyyy') AND db = ".$_REQUEST["head"]["db"]."");
			if ($x)
			{
				$head_id=$x;
			}
...
if (isset($_REQUEST["save_record"]))
{
	...
	Table_Update("bud_ru_cash_out_head", $keys, $v);
}
...
if (isset($_REQUEST["enabled"]))
{
	foreach ($_REQUEST["enabled"] as $k=>$v)
	{
		...
		Table_Update("bud_ru_cash_out_body", $keys,$vals);
	}
}
*/
if (isset($_REQUEST["save_record"]))
{
	...
	$p[":id"] = $head_id;
	$sql = rtrim(file_get_contents('sql/bud_ru_cash_out_head.sql'));
	$sql=stritr($sql,$p);
...
if (isset($_REQUEST["del_heads"]))
{
	...
	{
		Table_Update("bud_ru_cash_out_head", array("id"=>$k),null);
	}
}
...
$smarty->display('bud_ru_cash_out.html');
