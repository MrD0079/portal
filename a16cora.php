...
if (isset($_REQUEST["save"]))
{
	...
	Table_Update('a16cotp', $keys,$vals);
}
else if (isset($_REQUEST["saveurl"]))
{
	...
	Table_Update('a16courl', $keys,$vals);
}
else
{
	...
	{
		$sql=rtrim(file_get_contents('sql/a16cora.sql'));
		$sql=stritr($sql,$params);
		//echo $sql;
		$d = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
		foreach($d as $k=>$v)
		{
			$sql=rtrim(file_get_contents('sql/a16cora_photos.sql'));
