...
$sql = rtrim(file_get_contents('sql/val_bumvesna_report_eta_list.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents("sql/val_bumvesna_report.sql"));
$sql=stritr($sql,$params);
$list = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign("list", $list);

$sql=rtrim(file_get_contents("sql/val_bumvesna_report_itogi.sql"));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/val_bumvesna_files_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
...
$sql=rtrim(file_get_contents('sql/val_bumvesna_files_total_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getOne($sql);
...
$smarty->display('val_bumvesna_report.html');
