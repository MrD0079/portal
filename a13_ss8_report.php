...
$sql = rtrim(file_get_contents('sql/a13_ss8_report_eta_list.sql'));
$sql=stritr($sql,$params);
...
if (isset($_REQUEST["save"]))
{
	if (isset($_REQUEST["data"]))
	{
		...
		foreach($_REQUEST["data"] as $k=>$v)
		{
			...
			isset($v['bonus_dt2'])?$v['bonus_dt2']=OraDate2MDBDate($v['bonus_dt2']):null;
...
$sql=rtrim(file_get_contents("sql/a13_ss8_report.sql"));
$sql=stritr($sql,$params);
//echo $sql;
$list = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign("list", $list);

$sql=rtrim(file_get_contents("sql/a13_ss8_report_itogi.sql"));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_ss8_files_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
...
$sql=rtrim(file_get_contents('sql/a13_ss8_files_total_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getOne($sql);
...
$smarty->display('a13_ss8_report.html');
