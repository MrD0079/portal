...
audit("открыл akr_ag_report_stat","akr");
...
else
{
	...
	$sql=rtrim(file_get_contents('sql/akr_ag_report_stat_tz.sql'));
	$p = array(":id" => $r['id']);
	$sql=stritr($sql,$p);
	$akr_tz = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
	$smarty->assign('akr_tz', $akr_tz);

	$sql=rtrim(file_get_contents('sql/akr_ag_report_stat_head.sql'));
	$p = array(
		":tz_id" => $_REQUEST['tz'],
	...
	foreach ($z as $k=>$v)
	{
		$sql=rtrim(file_get_contents('sql/akr_ag_report_stat_detail.sql'));
		$p = array(
			":tz_id" => $v['id'],
			':sdt'=>"'".$_REQUEST['sdt']."'",
			':edt'=>"'".$_REQUEST['edt']."'"
		...
		$sql=rtrim(file_get_contents('sql/akr_ag_report_stat_detail_total.sql'));
		$p = array(
			":tz_id" => $v['id'],
			':sdt'=>"'".$_REQUEST['sdt']."'",
			':edt'=>"'".$_REQUEST['edt']."'"
		...
		foreach ($z1 as $k1=>$v1)
		{
			$sql=rtrim(file_get_contents('sql/akr_ag_report_stat_files.sql'));
...
$smarty->assign('akr_nets', $akr_nets);
	...
	$smarty->display('akr_ag_report_stat.html');
}
