...
";
				...
				{
					echo "<p>По одному клиенту в период акции может быть не более одной акционной накладной!!!</p>";
				}
			}
			else
			{
...
{
$fn=$db->getOne("select fn from a13_tz_files where id=".$k);
unlink("a13_tz_files/".$tn."/".$fn);
$fn=$db->query("delete from a13_tz_files where id=".$k);
}
}
...
if (isset($_FILES["new_fn"]))
{
	if ($_FILES["new_fn"]["name"]!="")
	{
		...
		if ($_FILES["new_fn"]['error']==0){
			Table_Update("a13_tz_files",$_REQUEST["new"],$_REQUEST["new"]);
		}
	}
}

$params=array(':dpt_id' => $_SESSION["dpt_id"],':tn'=>$tn,':tp_kat'=>1,':exp_list_without_ts' => 0,':exp_list_only_ts' => 0);
$sql=rtrim(file_get_contents('sql/a13_tz_files.sql'));
...
$sql = rtrim(file_get_contents('sql/a13_tz_process_eta_list.sql'));
$sql=stritr($sql,$params);
...
$sql = rtrim(file_get_contents('sql/a13_tz_process_nakl.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_tz_process_tp.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_tz_process_nakl_total.sql'));
$sql=stritr($sql,$params);
...
$smarty->display('a13_tz_process.html');
?>