...
audit("открыл atd_ag_report_stat","atd");
...
else
{
	...
	$sql=rtrim(file_get_contents('sql/atd_ag_report_stat_tz.sql'));
	$p = array(":id" => $r['id']);
	$sql=stritr($sql,$p);
	$atd_tz = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
	$smarty->assign('atd_tz', $atd_tz);

	$sql=rtrim(file_get_contents('sql/atd_ag_report_stat_head.sql'));
	$p = array(
		":tz_id" => $_REQUEST['tz'],
	...
	foreach ($z as $k=>$v)
	{
		$sql=rtrim(file_get_contents('sql/atd_ag_report_stat_detail.sql'));
		$p = array(
			":tz_id" => $v['id'],
			':sdt'=>"'".$_REQUEST['sdt']."'",
			':edt'=>"'".$_REQUEST['edt']."'"
		...
		$sql=rtrim(file_get_contents('sql/atd_ag_report_stat_detail_total.sql'));
		$p = array(
			":tz_id" => $v['id'],
			':sdt'=>"'".$_REQUEST['sdt']."'",
			':edt'=>"'".$_REQUEST['edt']."'"
		...
		$sql=stritr($sql,$p);
		//echo $sql;
		$z1t = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
		//print_r($z1);
		$z[$k]['detail_total']=$z1t;



		foreach ($z1 as $k1=>$v1)
		{
			$sql=rtrim(file_get_contents('sql/atd_ag_report_stat_files.sql'));
...
$smarty->assign('atd_nets', $atd_nets);
	...
	$smarty->display('atd_ag_report_stat.html');
}
