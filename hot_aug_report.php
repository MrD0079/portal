...
//ses_req();
	...
	$sql = rtrim(file_get_contents('sql/hot_aug_report_eta_list.sql'));
	$sql=stritr($sql,$params);
...
for ($i=1;$i<=2;$i++)
{
	...
	$sql=rtrim(file_get_contents("sql/hot_aug_report_".$i.".sql"));
	$sql=stritr($sql,$params);
	//echo $sql;
	...
	$sql_total=rtrim(file_get_contents("sql/hot_aug_report_".$i."_total.sql"));
...
$params[':ok_traid'] = $_REQUEST["ok_traid"];
$sql=rtrim(file_get_contents('sql/hot_aug_report_files.sql'));
$sql=stritr($sql,$params);
...
$params[':exp_list_only_ts'] = $_REQUEST["exp_list_only_ts"];
$params[':ok_traid'] = $_REQUEST["ok_traid"];
$sql=rtrim(file_get_contents('sql/hot_aug_report_files_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
...
$smarty->display('hot_aug_report.html');
