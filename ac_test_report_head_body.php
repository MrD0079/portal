...
$sql = rtrim(file_get_contents('sql/ac_test_report_members.sql'));
$p = array(':id' => $_REQUEST['id']);
$sql=stritr($sql,$p);
$memb = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);

//ac_test_report_test_res.sql
//ac_test_report_test_history.sql
...
$sql=rtrim(file_get_contents('sql/ac_test_report_test_res.sql'));

$p[':ac_test_id'] = $v["ac_test_logic"];
$sqle=stritr($sql,$p);
$tests = $db->getAll($sqle, null, null, null, MDB2_FETCHMODE_ASSOC);
$memb[$k]["test_res"]["logic"]=$tests;

$p[':ac_test_id'] = $v["ac_test_math"];
$sqle=stritr($sql,$p);
$tests = $db->getAll($sqle, null, null, null, MDB2_FETCHMODE_ASSOC);
$memb[$k]["test_res"]["math"]=$tests;

$sql=rtrim(file_get_contents('sql/ac_test_report_test_history.sql'));

$p[':ac_test_id'] = $v["ac_test_logic"];
$sqle=stritr($sql,$p);
$tests = $db->getAll($sqle, null, null, null, MDB2_FETCHMODE_ASSOC);
$memb[$k]["test_history"]["logic"]=$tests;

$p[':ac_test_id'] = $v["ac_test_math"];
$sqle=stritr($sql,$p);
$tests = $db->getAll($sqle, null, null, null, MDB2_FETCHMODE_ASSOC);
$memb[$k]["test_history"]["math"]=$tests;
...
foreach ($t1 as $k=>$v)
{
	foreach ($v as $k1=>$v1)
	{
		isset($t1[$k]['total']['c1'])?$t1[$k]['total']['c1']+=$v1['c1']:$t1[$k]['total']['c1']=0;
	}
}
...
foreach ($t11 as $k=>$v)
{
	...
	{
		isset($t11['total']['scores'][$k1])?$t11['total']['scores'][$k1]+=$v1:$t11['total']['scores'][$k1]=0;
	}
}
...
$smarty->display('ac_test_report_head_body.html');
