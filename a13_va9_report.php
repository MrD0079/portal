...
$sql = rtrim(file_get_contents('sql/a13_va9_report_eta_list.sql'));
$sql=stritr($sql,$params);
...
{
$fn=$db->getOne("select fn from a13_va9_files where id=".$k);
unlink("a13_va9_files/".$tn."/".$fn);
$fn=$db->query("delete from a13_va9_files where id=".$k);
}
}
...
if ($_FILES["new_fn"]["name"]!="")
{
	...
	if ($_FILES["new_fn"]['error']==0){
		...
		Table_Update("a13_va9_files",$_REQUEST["new"],$_REQUEST["new"]);
	}
}
}

$sql=rtrim(file_get_contents("sql/a13_va9_report.sql"));
$sql=stritr($sql,$params);
//echo $sql;
...
$sql=rtrim(file_get_contents("sql/a13_va9_report_itogi.sql"));
$sql=stritr($sql,$params);
//echo $sql;
...
$sql=rtrim(file_get_contents('sql/a13_va9_files.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_va9_files_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
...
$sql=rtrim(file_get_contents('sql/a13_va9_files_total_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getOne($sql);
//echo $sql;
...
$smarty->display('a13_va9_report.html');
