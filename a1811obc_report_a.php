    ...
    if (isset($_REQUEST["keys"])) {
        foreach ($_REQUEST["keys"] as $key => $val) {
            ...
            if ($vals["if1"]!=0) {
                /*$sql="SELECT COUNT (*)
                    FROM (
                    select h_tp_kod_data_nakl FROM a1808ms1_action_nakl
                    union
                    select h_tp_kod_data_nakl FROM a1808ms2_action_nakl
                    union
                    select h_tp_kod_data_nakl FROM a1807sc_action_nakl
                    union
                    select h_tp_kod_data_nakl FROM a1807sc2_action_nakl
                    union
