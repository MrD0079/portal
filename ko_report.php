...
$_SESSION["ok_chief"]=$_REQUEST["ok_chief"];
	...
	$sql=rtrim(file_get_contents('sql/ko_report.sql'));
	...
	$sql_total=rtrim(file_get_contents('sql/ko_report_total.sql'));
	...
	$sql_total_itogi=rtrim(file_get_contents('sql/ko_report_total_itogi.sql'));
...
$sql=rtrim(file_get_contents('sql/ko_files_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
if (isset($d)){foreach ($d as $k=>$v){$d1[$v["tn"]]["data_total"]=$v;}}
if (isset($d1)){$smarty->assign('file_list_b', $d1);}
$sql=rtrim(file_get_contents('sql/ko_files_total_total.sql'));
$sql=stritr($sql,$params);
