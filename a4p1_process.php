...
$sql = rtrim(file_get_contents('sql/a4p1_process_eta_list.sql'));
...
$sql = rtrim(file_get_contents('sql/a4p1_process_nakl.sql'));
$sql=stritr($sql,$params);
//echo $sql;
...
{
$fn=$db->getOne("select fn from a4p1_files where id=".$k);
unlink("a4p1_files/".$tn."/".$fn);
$fn=$db->query("delete from a4p1_files where id=".$k);
}
}
...
if ($_FILES["new_fn"]["name"]!="")
{
	...
	if ($_FILES["new_fn"]['error']==0){
		...
		Table_Update("a4p1_files",$_REQUEST["new"],$_REQUEST["new"]);
	}
}
}
...
$sql=rtrim(file_get_contents('sql/a4p1_files.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a4p1_process_tp.sql'));
$sql=stritr($sql,$params);
//echo $sql;
...
$params=array(':dpt_id' => $_SESSION["dpt_id"],':tn'=>$tn,':tp'=>0,':pt'=>$_REQUEST["pt"],':eta_list' => "'".$_REQUEST["eta_list"]."'");
$sql=rtrim(file_get_contents('sql/a4p1_process_nakl_total.sql'));
$sql=stritr($sql,$params);
$sql=stritr($sql,$params);
...
$smarty->display('a4p1_process.html');
?>