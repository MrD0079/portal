...
audit("открыл azl_contr_avk","azl");

if (isset($_REQUEST["save"])&&isset($_REQUEST["data"]))
{
	foreach ($_REQUEST["data"] as $k=>$v)
	{
		Table_Update("azl_contr_avk", array("id"=>$k),$v);
	}
}
...
if (isset($_REQUEST["del"]))
{
	...
	{
		Table_Update("azl_contr_avk", array("id"=>$k),null);
	}
}
...
if (isset($_REQUEST["send"]))
{
	foreach ($_REQUEST["send"] as $k=>$v)
	{
		...
		{
			$subj='Информация для доступа к функционалу акции "Дегустация Жувиленд"';
...
{
	Table_Update("azl_contr_avk", $_REQUEST["new"],$_REQUEST["new"]);
}
...
$smarty->display('azl_contr_avk.html');
