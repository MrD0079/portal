...
if (isset($_REQUEST["save"]))
{
	...
	if (isset($_REQUEST["bonus_list"]))
	{
		foreach($_REQUEST["bonus_list"] as $k=>$v)
		{
			$keys = array('id'=>$k);
			Table_Update ('act_action_nakl_b', $keys, $v);
		}
	}
...
{
$params[':nakl_id']=$v['id'];
$sql=rtrim(file_get_contents("sql/'.$_REQUEST['act'].'_report_a_bonus_list.sql"));
$sql=stritr($sql,$params);
$list_b = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$list[$k]["bonus_list"]=$list_b;
}
