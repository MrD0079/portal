...
";
				...
				if ($c==0)
				{
					echo "<p style='color:red'>ѕо одному клиенту в период акции может быть не более двух бонусных коробок по каждому из трех видов печень€ и не более 6 бонусных коробок по “ѕ !!!</p>";
...
{
$fn=$db->getOne("select fn from a13_sp8_files where id=".$k);
unlink("a13_sp8_files/".$tn."/".$fn);
$fn=$db->query("delete from a13_sp8_files where id=".$k);
}
}
...
if (isset($_FILES["new_fn"]))
{
	if ($_FILES["new_fn"]["name"]!="")
	{
		...
		if ($_FILES["new_fn"]['error']==0){
			Table_Update("a13_sp8_files",$_REQUEST["new"],$_REQUEST["new"]);
		}
	}
}
...
$sql = rtrim(file_get_contents('sql/a13_sp8_process_eta_list.sql'));
$sql=stritr($sql,$params);
...
$sql = rtrim(file_get_contents('sql/a13_sp8_process_nakl.sql'));
$sql=stritr($sql,$params);
//echo $sql;
...
$sql=rtrim(file_get_contents('sql/a13_sp8_process_tp.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_sp8_process_nakl_total.sql'));
$sql=stritr($sql,$params);
...
$smarty->display('a13_sp8_process.html');
?>