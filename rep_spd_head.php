...
else
{
	...
	if (isset($_REQUEST["generate"]))
	{
		...
		Table_Update('rep_spd_dt', $keys,$keys);
	}
	$sql = rtrim(file_get_contents('sql/rep_spd_head.sql'));
	$sql=stritr($sql,$p);
	$r = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
	$smarty->assign('rep_spd_head', $r);
	$sql = rtrim(file_get_contents('sql/rep_spd_generate_status.sql'));
	$sql=stritr($sql,$p);
	//echo $sql;
	$r = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
	$smarty->assign('status', $r);
	//print_r($r);
	$smarty->display('rep_spd_head.html');
}
