...
$smarty->assign('tr_pt_name', $db->getOne("select name from tr where id=".$_REQUEST["tr"]));
$smarty->assign('q_name', $db->getOne("select name from tr_test_qa where id_num=".$_REQUEST["q_id"]));


$sql=rtrim(file_get_contents('sql/tr_pt_rep_test_error_detail.sql'));
$sql=stritr($sql,$params);
$r = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign('d', $r);

$smarty->display('tr_pt_rep_test_error_detail.html');
