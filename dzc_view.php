...
$sql=rtrim(file_get_contents('sql/dzc_view.sql'));
...
$sql=rtrim(file_get_contents('sql/dzc_view_chat.sql'));
$params=array(':id' => $_REQUEST["id"]);
$sql=stritr($sql,$params);
$chat = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$d["chat"]=$chat;
$url="";
$d["head"]["comm8"]=mb_convert_encoding($d["head"]["comm"],"UTF-8","Windows-1251");
$url.="Заявка на компенсацию дистрибутору №".$d["head"]["id"]." от ".$d["head"]["created_dt"];
$url.="\nСогласовано:";
...
{
$a = $smarty->display('dzc_view.html');
}
else
{
$a = $smarty->fetch('dzc_view.html');
...
$dompdf->load_html($b);
$dompdf->render();
$dompdf->stream("dzc".$_REQUEST["id"].".pdf");
}
else
{
$dompdf1 = new DOMPDF();
