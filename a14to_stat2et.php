...
if (isset($_REQUEST["save"]))
{
	...
	if (isset($_REQUEST["data"]))
	{
		foreach ($_REQUEST["data"] as $key => $val)
		{
			...
			Table_Update ('a14tozst', $keys, $values);
		}
	}
}
	...
	if (isset($_REQUEST['generate']))
	{
		if ($_REQUEST["rep_type"]=="brief")
		{
			...
			$sql=rtrim(file_get_contents('sql/a14to_stat2et_'.$_REQUEST['by_who'].'1.sql'));
			$sql=stritr($sql,$params);
			$t1 = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
			$sql=rtrim(file_get_contents('sql/a14to_stat2et_'.$_REQUEST['by_who'].'2.sql'));
			$sql=stritr($sql,$params);
			$t2 = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
			$sql=rtrim(file_get_contents('sql/a14to_stat2et_'.$_REQUEST['by_who'].'3.sql'));
			$sql=stritr($sql,$params);
		...
		{
			$sql=rtrim(file_get_contents('sql/a14to_stat2et_detailed.sql'));
			$sql=stritr($sql,$params);
			$t = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
			$smarty->assign('d', $t);
		}
		$sql=rtrim(file_get_contents('sql/a14to_stat2et_total.sql'));
		$sql=stritr($sql,$params);
		$t = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
		$smarty->assign('tt', $t);
		$sql=rtrim(file_get_contents('sql/a14to_stat2et_total1.sql'));
		$sql=stritr($sql,$params);
		$t1 = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
		$smarty->assign('tt1', $t1);
		$sql=rtrim(file_get_contents('sql/a14to_stat2et_total2.sql'));
		$sql=stritr($sql,$params);
		$t2 = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
		$smarty->assign('tt2', $t2);
		$sql=rtrim(file_get_contents('sql/a14to_stat2et_total3.sql'));
		$sql=stritr($sql,$params);
		$t3 = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
		$smarty->assign('tt3', $t3);
	}
	$smarty->display('a14to_stat2et.html');
?>