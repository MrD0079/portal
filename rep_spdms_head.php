...
else
{
	...
	if (isset($_REQUEST["generate"]))
	{
		...
		Table_Update('rep_spdms_dt', $keys,$keys);
	}
	$sql = rtrim(file_get_contents('sql/rep_spdms_head.sql'));
	$sql=stritr($sql,$p);
	$r = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
	$smarty->assign('rep_spdms_head', $r);
	$sql = rtrim(file_get_contents('sql/rep_spdms_generate_status.sql'));
	$sql=stritr($sql,$p);
	//echo $sql;
