...
if (isset($_REQUEST["add"]))
{
	if (isset($_REQUEST["keys"]))
	{
		foreach ($_REQUEST["keys"] as $key => $val)
		{
			...
			if ($vals["if1"]==1)
			{
				$sql="SELECT COUNT (*) FROM a13_pk_action_nakl WHERE h_tp_kod_data_nakl IN (SELECT h_tp_kod_data_nakl FROM a13_pk WHERE tp_kod = (SELECT tp_kod FROM a13_pk WHERE h_tp_kod_data_nakl = '".$key."'))";
				//echo $sql;
				$c=$db->getOne($sql);
				if ($c==0)
				{
				...
				{
					echo "<p>По одному клиенту за период акции может быть только одна акционная накладная!!!</p>";
				}
			}
			else
			{
...
{
$fn=$db->getOne("select fn from a13_pk_files where id=".$k);
unlink("a13_pk_files/".$tn."/".$fn);
$fn=$db->query("delete from a13_pk_files where id=".$k);
}
}
...
if (isset($_FILES["new_fn"]))
{
	if ($_FILES["new_fn"]["name"]!="")
	{
		...
		if ($_FILES["new_fn"]['error']==0){
			Table_Update("a13_pk_files",$_REQUEST["new"],$_REQUEST["new"]);
		}
	}
}

$params=array(':dpt_id' => $_SESSION["dpt_id"],':tn'=>$tn,':tp_kat'=>1,':exp_list_without_ts' => 0,':exp_list_only_ts' => 0);
$sql=rtrim(file_get_contents('sql/a13_pk_files.sql'));
...
$sql = rtrim(file_get_contents('sql/a13_pk_process_eta_list.sql'));
$sql=stritr($sql,$params);
...
$sql = rtrim(file_get_contents('sql/a13_pk_process_nakl.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_pk_process_tp.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_pk_process_nakl_total.sql'));
$sql=stritr($sql,$params);
...
$smarty->display('a13_pk_process.html');
?>