/* Formatted on 20.01.2017 16:13:03 (QP5 v5.252.13127.32867) */
  SELECT rmkk,
         rmkk_name,
         mkk_ter,
         mkk_name,
    ...
    FROM (  SELECT m.rmkk,
                   ...
                   ptm.ok_nm,
                   ptm.ok_fm,
                   TO_CHAR (ptm.ok_tm_lu, 'dd.mm.yyyy hh24:mi:ss') ok_tm_lu,
                   TO_CHAR (ptm.ok_nm_lu, 'dd.mm.yyyy hh24:mi:ss') ok_nm_lu,
                   TO_CHAR (ptm.ok_fm_lu, 'dd.mm.yyyy hh24:mi:ss') ok_fm_lu,
                   pnm.oknm_nm,
                   pnm.oknm_fm,
                   TO_CHAR (pnm.oknm_nmlu, 'dd.mm.yyyy hh24:mi:ss') oknm_nmlu,
                   TO_CHAR (pnm.oknm_fmlu, 'dd.mm.yyyy hh24:mi:ss') oknm_fmlu,
              ...
              FROM (  SELECT fn_getname (n2.tn_rmkk) rmkk_name,
                             ...
                             list.month,
                             list.id_net,
                             SUM (CASE WHEN m1.payment_type = 1 THEN m1.total END)
                                total1
                       ...
                       WHERE     list.id_net = m1.id_net(+)
                             ...
                             AND (    (   list.mkk_ter IN (SELECT slave
                                       ...
                                       OR (SELECT is_fin_man
                                             FROM user_list
                                            WHERE tn = :tn) = 1)
                                  AND DECODE ( :tn_rmkk, 0, n2.tn_rmkk, :tn_rmkk) =
                                         n2.tn_rmkk
                                  AND DECODE ( :tn_mkk, 0, list.mkk_ter, :tn_mkk) =
                                         list.mkk_ter)
                    GROUP BY n2.tn_rmkk,
                             list.mkk_ter,
                             n2.net_name,
                   ...
                   (  SELECT m.mkk_ter, m.id_net, SUM (id.summa) total
                        FROM nets_plan_month m,
                             invoice_detail id,
                             invoice i,
                             nets_plan_month ms,
