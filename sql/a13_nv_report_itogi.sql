/* Formatted on 25.12.2013 15:55:39 (QP5 v5.227.12220.39724) */
SELECT COUNT (DISTINCT tp_kod) cnt,
       SUM (DECODE (rost, 0, 0, 1)) act,
       ...
       SUM (bonusplan) bonusplan,
         DECODE (
            NVL (SUM (salesdec), 0),
            0, 0,
              (SUM (DECODE (bonus_dt1, NULL, NULL, bonus_sum1)))
            / SUM (salesdec))
       * 100
          zat
  FROM (  SELECT d.fio_eta,
                 ...
                 TO_CHAR (tp.ok_chief_date, 'dd.mm.yyyy hh24:mi:ss')
                    ok_chief_date,
                 TO_CHAR (tp.ok_traid_date, 'dd.mm.yyyy hh24:mi:ss')
                    ok_traid_date,
                 tp.ok_chief,
                 tp.ok_traid,
                 tp.ok_traid_fio,
