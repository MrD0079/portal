/* Formatted on 08/01/2015 9:37:24 (QP5 v5.227.12220.39724) */
  SELECT key,
         SUM (VALUE) VALUE,
         SUM (tp_kod_cnt) tp_kod_cnt,
         SUM (tp_kod_cnt) / SUM (VALUE) * 100 perc_pokr_sto
    FROM (  SELECT parent_tn key, SUM (VALUE) VALUE, SUM (tp_kod_cnt) tp_kod_cnt
              FROM (  SELECT parent_tn,
                             h_fio_eta key,
                             AVG (VALUE) VALUE,
                             COUNT (DISTINCT tp_kod_key /*|| visitdate*/) tp_kod_cnt
         ...
         AND */ u.tab_num = t.tab_num
                                       ...
                                       AND t.visitdate BETWEEN TO_DATE (:sd,
                                                                        'dd.mm.yyyy')
                                                           AND TO_DATE (:ed,
                                                                        'dd.mm.yyyy')
                                       AND DECODE (:eta_list,
                                                   '', t.h_fio_eta,
                                                   :eta_list) = t.h_fio_eta
