/* Formatted on 03.08.2017 11:46:18 (QP5 v5.252.13127.32867) */
  SELECT dbf.id,
         dbf.name,
         ...
         DECODE (SUM (NVL (fd.plan_val, 0) + NVL (fd.planl_val, 0)), 0, 0, SUM (fakt.fakt) / SUM ( (NVL (fd.plan_val, 0) + NVL (fd.planl_val, 0))) * 100) fakt_perc,
    ...
    FROM (  SELECT l.tn,
              ...
              FROM (SELECT *
                   ...
                   user_list u,
                   bud_funds f
             WHERE b.db(+) = l.tn AND l.tn = u.tn AND NVL (u.is_kk, 0) = :kk AND f.dpt_id = :dpt_id AND f.planned = 1
          ORDER BY l.tn, f.id) dbf,
         (SELECT *
            FROM bud_funds_limits_f
           WHERE dt = TO_DATE ( :month_list, 'dd.mm.yyyy') AND dpt_id = :dpt_id AND kk = :kk) fd,
         (  SELECT z.tn, z.funds, SUM (z_fakt.val_number) fakt
             ...
             WHERE     z.tn = u.tn
                   ...
                   AND u.dpt_id = :dpt_id
                   AND TRUNC (z.dt_start, 'mm') = TO_DATE ( :month_list, 'dd.mm.yyyy')
         ...
         (SELECT *
           ...
           WHERE ft.dt = TO_DATE ( :month_list, 'dd.mm.yyyy') AND ft.dpt_id = :dpt_id AND ft.kk = :kk AND ft.dt = h.dt AND ft.dpt_id = h.dpt_id AND ft.kk = h.kk AND h.ok_dpu = 1) accepted
   WHERE fd.fund(+) = dbf.id AND fakt.funds(+) = dbf.id AND dbf.tn = fd.db(+) AND dbf.tn = fakt.tn(+) AND accepted.fund(+) = dbf.id
GROUP BY dbf.id,
         dbf.name,
         accepted.plan_val,
         accepted.planl_val
