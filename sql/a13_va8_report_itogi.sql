/* Formatted on 03.09.2013 10:39:00 (QP5 v5.227.12220.39724) */
SELECT COUNT (*) c,
       ...
       DECODE (
          NVL (SUM (DECODE (is_act, 1, sum07 + sum08, 0)), 0),
          0, 0,
            SUM (DECODE (is_act, 1, fakt_bonus, 0))
          / SUM (DECODE (is_act, 1, sum07 + sum08, 0))
          * 100)
          act_uah,
       SUM (sum07) sum07,
       SUM (sum08) sum08,
       DECODE (NVL (SUM (sum07), 0),
               0, 0,
               SUM (sum08) / SUM (sum07) * 100 - 100)
          rost_perc,
       AVG (
          (SELECT SUM (bonus)
             FROM a13_va8_files st
            WHERE     st.tn IN
                         (SELECT slave
                                 FROM full
                                WHERE master =
       ...
       AVG (
          (SELECT SUM (bonus)
            ...
            WHERE     st.tn IN
                  ...
                  AND ok_traid = 1))
          files_bonus_traid
  FROM (  SELECT z1.*, DECODE (SIGN (rost_perc - 40), -1, 0, 1) is_act
            FROM (SELECT z.*,
                         DECODE (
                            SIGN (
                               DECODE (NVL (sum07, 0),
           ...
           WHERE DECODE (:is_act,  1, 0,  2, 1,  3, 0) =
                    DECODE (:is_act,
                            1, 0,
                            DECODE (SIGN (rost_perc - 40), -1, 0, 1))
        ORDER BY fio_ts,
                 fio_eta,
                 tp_ur,
                 tp_addr)