/* Formatted on 18.04.2014 9:46:45 (QP5 v5.227.12220.39724) */
SELECT COUNT (DISTINCT tp_kod) cnt_tp,
       SUM (DECODE (is_act, 0, 0, bonus_sum)) bonus_sum,
       SUM (DECODE (bonus_dt, NULL, NULL, 1)) bonus_dt_cnt,
       SUM ( (SELECT DECODE (lu, NULL, 0, 1)
          ...
          ok_chief,
       DECODE (
          NVL (SUM (DECODE (is_act, 0, 0, mar_apr)), 0),
          0, 0,
            SUM (DECODE (is_act, 0, 0, bonus_sum))
          / SUM (DECODE (is_act, 0, 0, mar_apr))
          * 100)
          zat,
       SUM (DECODE (is_act, 0, 0, jan_feb)) jan_feb,
       SUM (DECODE (is_act, 0, 0, mar_apr)) mar_apr,
       SUM (DECODE (is_act, 0, 0, delta)) delta,
       SUM (DECODE (is_act, 0, 0, max_bonus)) max_bonus,
       SUM (is_act) is_act
  FROM (SELECT z.*,
               NVL (
                  CASE
               ...
               NVL (
                  CASE
                     ...
                     THEN
                        CASE
                           WHEN delta >= 2000 AND delta < 3000 THEN 240
                           WHEN delta >= 3000 AND delta < 5000 THEN 360
                           WHEN delta > 5000 THEN 600
                        END
                     WHEN     jan_feb > 5000
                          AND jan_feb <= 7000
                          AND delta >= 3000
                     THEN
                        CASE
                           WHEN delta >= 3000 AND delta < 5000 THEN 360
                           WHEN delta >= 5000 AND delta < 7000 THEN 600
                           WHEN delta > 7000 THEN 840
                        END
                     WHEN     jan_feb > 6000
                          AND jan_feb <= 10000
                          AND delta >= 5000
                     THEN
                        CASE
                           WHEN delta >= 5000 AND delta < 7000 THEN 600
                           WHEN delta >= 7000 AND delta < 10000 THEN 840
                           WHEN delta > 10000 THEN 1200
                        END
                     WHEN     jan_feb > 10000
                          AND jan_feb <= 15000
                          AND delta >= 7000
                     THEN
                        CASE
                           WHEN delta >= 7000 AND delta < 10000 THEN 840
                           WHEN delta >= 10000 AND delta < 15000 THEN 1200
                           WHEN delta > 15000 THEN 1800
                        END
                     WHEN     jan_feb > 15000
                          AND jan_feb <= 20000
                          AND delta >= 10000
                     THEN
                        CASE
                           WHEN delta >= 10000 AND delta < 15000 THEN 1200
                           WHEN delta >= 15000 AND delta < 20000 THEN 1800
                           WHEN delta > 20000 THEN 2400
                        END
                     WHEN     jan_feb > 20000
                          AND jan_feb <= 30000
                          AND delta >= 15000
          ...
          FROM (SELECT d.fio_eta,
                       ...
                       - NVL (d.jan_feb, 0)
                          delta,
                       tp.bonus_sum,
                       TO_CHAR (tp.bonus_dt, 'dd.mm.yyyy') bonus_dt,
                  ...
                  FROM a1404bum d,
                       ...
                       (  SELECT t1.tp_kod,
                           ...
                           WHERE     st.tab_num = t1.tab_num
                                 ...
                                 AND t2.if1 = 1
                                 AND t1.data BETWEEN TO_DATE ('01.03.2014',
                                                              'dd.mm.yyyy')
                                                 AND TO_DATE ('30.04.2014',
                                                              'dd.mm.yyyy')
              ...
              OR (SELECT NVL (is_traid, 0)
                   ...
                   WHERE tn = :tn) = 1)
                       ...
                       AND DECODE (:ok_bonus,  1, 0,  2, 1,  3, 0) =
                              DECODE (:ok_bonus,
                                      1, 0,
                                      2, DECODE (tp.bonus_dt, NULL, 0, 1),
                                      3, DECODE (tp.bonus_dt, NULL, 0, 1))
 ...
 WHERE DECODE (:is_act,  1, 0,  2, 1,  3, 0) = DECODE (:is_act, 1, 0, is_act)