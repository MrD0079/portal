/* Formatted on 18/05/2015 19:38:19 (QP5 v5.227.12220.39724) */
  SELECT SUM (zay_compens_distr) zay_compens_distr,
         ...
         fil_id,
         name
    FROM (/* Formatted on 22/09/2015 13:16:57 (QP5 v5.227.12220.39724) */
  SELECT tf.fil_id,
         tf.name,
         /*tf.db_list,*/
         zay.z_plan,
    ...
    FROM (  SELECT f.id fil_id,
                       ...
                       WHERE     u.tab_num = s.ts_tab_num
                             ...
                             AND (   u.tn IN (SELECT slave
                                  ...
                                  OR (SELECT NVL (is_traid_kk, 0)
                                        FROM user_list
                                       WHERE tn = :tn) = 1)
                             AND DECODE (:eta_list, '', s.h_fio_eta, :eta_list) =
                                    s.h_fio_eta
         ...
         (  SELECT zp.fil fil_id,
                   ...
                   SUM (NVL (sv.bonus_fakt, 0) + NVL (sv.fixed_fakt, 0))
                      fakt_distr,
                   SUM (s.skidka * s.summa / 100) skidka_val
