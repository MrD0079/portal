...
SELECT COUNT (DISTINCT tz.id) tz,
  ...
  FROM azl_tz tz,
       ...
       (SELECT TO_CHAR (dt.dt, 'dd.mm.yyyy') dt,
          ...
          FROM (SELECT dt, tz_id FROM azl_tz_ag_report
                UNION
                SELECT dt, tz_id FROM azl_tz_report
                UNION
                SELECT dt, tz FROM azl_files
                UNION
 ...
 WHERE     tz.city = c.id(+)
       ...
       AND tz.contr_avk = DECODE (:azl_contr_avk, 0, tz.contr_avk, :azl_contr_avk)
       AND tz.contr_ag = DECODE (:azl_contr_ag, 0, tz.contr_ag, :azl_contr_ag)
       AND tz.id = dt.tz_id(+)
