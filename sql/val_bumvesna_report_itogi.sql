...
SELECT COUNT (DISTINCT an.H_TP_KOD_DATA_NAKL) cnt_nakl,
       ...
       SUM (d.summa) summa,
         ...
         SUM (d.packveskonf) packveskonf,
         SUM (d.SUMmshok) SUMmshok,
         SUM (d.SUMMvespech) SUMMvespech,

       SUM (an.ok_chief) ok_chief,
       SUM (an.bonus_sum) bonus_sum,
       AVG ( (SELECT SUM (bonus) bonus
                FROM val_bumvesna_files f, user_list st
               WHERE     f.ok_traid = 1
                     AND f.tn = st.tn
  ...
  FROM val_bumvesna d,
       val_bumvesna_action_nakl an,
       user_list st,
       val_bumvesna_tp_select tp
