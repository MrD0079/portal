/* Formatted on 13.01.2018 18:47:29 (QP5 v5.252.13127.32867) */
  SELECT ff.id,
         ff.name,
         ff.sort,
         COUNT (DISTINCT z.id) z_cnt
    FROM bud_ru_zay z,
         bud_ru_zay_ff zf,
         bud_ru_st_ras st,
         bud_ru_ff ff
   WHERE     z.id = zf.z_id
         ...
         AND z.id IN (SELECT DISTINCT id
                        FROM (  SELECT z.*,
                                  ...
                                  FROM (SELECT bud_ru_zay.id,
                                               ...
                                               TO_CHAR (bud_ru_zay.created,
                                                  ...
                                                  created,
                                               TO_CHAR (bud_ru_zay.dt_start,
                                                        'dd.mm.yyyy')
                                                  dt_start,
                                               TO_CHAR (bud_ru_zay.dt_end,
                                                        'dd.mm.yyyy')
                                                  dt_end,
                                               TO_CHAR (bud_ru_zay.report_data,
                                                        'dd.mm.yyyy')
                                                  report_data,
                                               ...
                                               fn_getname (bud_ru_zay_executors.tn)
                                                  executor_name,
                                               bud_ru_zay_executors.execute_order,
                                               bud_ru_zay_executors.pos_name
                                                  executor_pos_name,
                                          ...
                                          FROM bud_ru_zay,
                                               ...
                                               (SELECT sze.*,
                                                       szu.pos_name,
                                                       szu.department_name
                                                  FROM bud_ru_zay_executors sze,
                                                       user_list szu
                                                 WHERE sze.tn = szu.tn)
                                               bud_ru_zay_executors,
                                               accept_types zat,
                                               user_list u,
                                               user_list u1,
                                               user_list u2,
                                         ...
                                         WHERE     (SELECT NVL (tu, 0)
                                                      FROM bud_ru_st_ras
                                                     WHERE id = bud_ru_zay.kat) =
                                                      :tu
                                               AND bud_ru_zay.id_net = n.id_net(+)
                                               /*dyn_flt*/
                                               AND bud_ru_zay.payment_type =
                                                      pt.id(+)
                                               ...
                                               AND bud_ru_zay.id =
                                                      bud_ru_zay_accept.z_id(+)
                                               AND bud_ru_zay.id =
                                                      bud_ru_zay_executors.z_id(+)
                                               AND bud_ru_zay_accept.accepted =
                                                      zat.id(+)
                              ...
                              ORDER BY DECODE ( :orderby,
                                               1, created_dt,
                                               2, current_accepted_date),
                                       id,
                                       accept_order,
                                       chat_time_d))
GROUP BY ff.id, ff.name, ff.sort
ORDER BY ff.sort, ff.name