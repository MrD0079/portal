/* Formatted on 20/11/2015 12:10:39 PM (QP5 v5.252.13127.32867) */
SELECT SUM (no_budget_val) no_budget,
       SUM (prev_year_fakt) prev_year_fakt,
       SUM (year_plan) year_plan,
       SUM (year_plan_ng) year_plan_ng,
       SUM (year_plan_coffee) year_plan_coffee,
       SUM (budget_sdu) budget_sdu,
       DECODE (SUM (NVL (year_plan_prev, 0)),
               0, 0,
               (SUM (year_plan) / SUM (year_plan_prev) - 1) * 100)
          perc_rost,
       DECODE (SUM (NVL (year_plan_prev_ng, 0)),
               0, 0,
               (SUM (year_plan_ng) / SUM (year_plan_prev_ng) - 1) * 100)
          perc_rost_ng,
       DECODE (
          SUM (NVL (year_plan_prev_coffee, 0)),
          0, 0,
          (SUM (year_plan_coffee) / SUM (year_plan_prev_coffee) - 1) * 100)
          perc_rost_coffee,
       SUM (budget) budget,
       SUM (budget_ng) budget_ng,
       SUM (budget_coffee) budget_coffee,
       SUM (budget_dog) budget_dog,
       SUM (budget_dog_ng) budget_dog_ng,
       SUM (budget_dog_coffee) budget_dog_coffee,
       DECODE (SUM (NVL (budget_prev, 0)),
               0, 0,
               SUM (budget) / SUM (budget_prev) * 100 - 100)
          budget_rost,
       DECODE (SUM (NVL (budget_ng_prev, 0)),
               0, 0,
               SUM (budget_ng) / SUM (budget_ng_prev) * 100 - 100)
          budget_rost_ng,
       DECODE (SUM (NVL (budget_coffee_prev, 0)),
               0, 0,
               SUM (budget_coffee) / SUM (budget_coffee_prev) * 100 - 100)
          budget_rost_coffee,
       DECODE (SUM (NVL (budget_dog_prev, 0)),
               0, 0,
               SUM (budget_dog) / SUM (budget_dog_prev) * 100 - 100)
          budget_dog_rost,
       DECODE (SUM (NVL (budget_dog_ng_prev, 0)),
               0, 0,
               SUM (budget_dog_ng) / SUM (budget_dog_ng_prev) * 100 - 100)
          budget_dog_rost_ng,
       DECODE (
          SUM (NVL (budget_dog_coffee_prev, 0)),
          0, 0,
          SUM (budget_dog_coffee) / SUM (budget_dog_coffee_prev) * 100 - 100)
          budget_dog_rost_coffee,
       DECODE (SUM (NVL (year_plan, 0)),
               0, 0,
               SUM (budget) / SUM (year_plan) * 100)
          perc_zatr,
       DECODE (SUM (NVL (year_plan, 0)),
               0, 0,
               SUM (budget_ng) / SUM (year_plan) * 100)
          perc_zatr_ng,
       DECODE (SUM (NVL (year_plan, 0)),
               0, 0,
               SUM (budget_coffee) / SUM (year_plan) * 100)
          perc_zatr_coffee,
       DECODE (SUM (NVL (year_plan, 0)),
               0, 0,
               SUM (budget_dog) / SUM (year_plan) * 100)
          perc_zatr_dog,
       DECODE (SUM (NVL (year_plan, 0)),
               0, 0,
               SUM (budget_dog_ng) / SUM (year_plan) * 100)
          perc_zatr_dog_ng,
       DECODE (SUM (NVL (year_plan, 0)),
               0, 0,
               SUM (budget_dog_coffee) / SUM (year_plan) * 100)
          perc_zatr_dog_coffee,
       SUM (ok_rmkk_tmkk) ok_rmkk_tmkk,
       SUM (ok_dpu) ok_dpu,
       SUM (ok_fin_man) ok_fin_man,
       SUM (du_complete_val) du_complete,
       SUM (ok_fin_man_dog) ok_fin_man_dog
  FROM (  SELECT n.tn_rmkk,
                 ...
                 DECODE (NVL (budget.budget_ng_prev, 0),
                    ...
                    budget_rost_ng,
                 DECODE (
                    NVL (budget.budget_coffee_prev, 0),
                    0, 0,
                      budget.budget_coffee / budget.budget_coffee_prev * 100
                    - 100)
                    budget_rost_coffee,
                 DECODE (
                    NVL (budget.budget_dog_prev, 0),
                    0, 0,
                    budget.budget_dog / budget.budget_dog_prev * 100 - 100)
                    budget_dog_rost,
                 DECODE (
                    NVL (budget.budget_dog_ng_prev, 0),
                    0, 0,
                      budget.budget_dog_ng / budget.budget_dog_ng_prev * 100
                    - 100)
                    budget_dog_rost_ng,
                 DECODE (
                 ...
                 DECODE (NVL (y.sales_coffee, 0),
                    ...
                    perc_zatr_coffee,
                 DECODE (NVL (y.sales, 0),
                         0, 0,
                         budget.budget_dog / y.sales * 100)
                    perc_zatr_dog,
                 DECODE (NVL (y.sales_ng, 0),
                         0, 0,
