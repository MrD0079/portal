/* Formatted on 27.03.2013 16:55:53 (QP5 v5.163.1008.3004) */
SELECT SUM (fact_ball) fact_ball
  FROM (  SELECT SUM (wm.akb) m_akb,
                 SUM (wbm.akb) bm_akb,
           ...
           WHERE     v.m = TO_DATE (:dt, 'dd.mm.yyyy')
                 ...
                 AND (u.tn IN
                         (SELECT slave
                            FROM full
                           WHERE master =
                                    DECODE (:w4u_process_ts_list,
                                            0, DECODE (:w4u_process_tm_list, 0, DECODE (:w4u_process_rm_list, 0, :tn, :w4u_process_rm_list), :w4u_process_tm_list),
                                            :w4u_process_ts_list))
