/* Formatted on 22.08.2017 13:16:39 (QP5 v5.252.13127.32867) */
  SELECT tf.fil_id,
         tf.name,
         /*tf.db_list,*/
         zay.z_plan,
         ...
         NVL (zay.compens_distr, 0) + NVL (act_local.compens_distr, 0) + NVL (act.compens_distr, 0) + NVL (svs.compens_distr, 0) + NVL (svs_new.compens_distr, 0) compens_distr,
         ...
         NVL (zay.fakt_distr, 0) + NVL (act_local.fakt_distr, 0) + NVL (act.fakt_distr, 0) + NVL (svs.fakt_distr, 0) + NVL (svs_new.fakt_distr, 0) fakt_distr,
    ...
    FROM (  SELECT f.id fil_id,
          ...
          GROUP BY f.id,
                   ...
                   tf.bud_id,
                   c.period) tf,
         (/* Formatted on 22.08.2017 15:29:55 (QP5 v5.252.13127.32867) */
  SELECT fil_id,
         cost_assign_month period,
         COUNT (*) c,
         SUM (z_plan) z_plan,
         SUM (z_fakt) z_fakt,
    ...
    FROM (  SELECT z.fil fil_id,
                   ...
                   END
                      compens_distr,
                   CASE WHEN via_db = 1 THEN NULL ELSE CASE WHEN by_goods = 0 THEN z_fakt ELSE z_fakt END END fakt_distr,
          ...
          ORDER BY period,
                   ...
                   st_name,
                   z.id)
GROUP BY fil_id, cost_assign_month) zay,
         (/* Formatted on 23.08.2017 10:59:41 (QP5 v5.252.13127.32867) */
  SELECT fil_id,
         cost_assign_month period,
         SUM (c) c,
         SUM (summa) summa,
...
GROUP BY fil_id, cost_assign_month) act_local,
         (  SELECT fil_id,
                   period,
                   SUM (sales) sales,
                   SUM (tp) tp,
         ...
         (  SELECT zp.fil fil_id,
                   ...
                   SUM (  (NVL (sv.bonus_fakt, 0) + NVL (sv.fixed_fakt, 0))
                        * CASE
                          ...
                          END)
                      compens_distr,
                   SUM (NVL (sv.bonus_fakt, 0) + NVL (sv.fixed_fakt, 0)) fakt_distr,
                                               ...
                                               AND (SELECT rep_accepted
                                                     ...
                                                     WHERE     z_id = z.id
                                                           AND accept_order = DECODE (NVL ( (SELECT MAX (accept_order)
                                                                                      ...
                                                                                      (SELECT MAX (accept_order)
                                                                                         FROM bud_ru_zay_accept
                                                                                        WHERE z_id = z.id AND rep_accepted = 2))) = 1
   ...
   WHERE tf.fil_id = zay.fil_id(+) AND tf.fil_id = act_local.fil_id(+) AND tf.fil_id = act.fil_id(+) AND tf.fil_id = svs.fil_id(+) AND tf.fil_id = svs_new.fil_id(+) AND tf.period = zay.period(+) AND tf.period = act_local.period(+) AND tf.period = act.period(+) AND tf.period = svs.period(+) AND tf.period = svs_new.period(+)
