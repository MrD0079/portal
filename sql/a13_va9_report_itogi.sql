/* Formatted on 19.09.2013 16:47:52 (QP5 v5.227.12220.39724) */
SELECT COUNT (*) c,
       ...
       DECODE (
          NVL (SUM (DECODE (is_act, 1, sum08 + sum09, 0)), 0),
          0, 0,
            SUM (DECODE (is_act, 1, fakt_bonus, 0))
          / SUM (DECODE (is_act, 1, sum08 + sum09, 0))
          * 100)
          act_uah,
       SUM (sum08) sum08,
       SUM (sum09) sum09,
       DECODE (NVL (SUM (sum08), 0),
               0, 0,
               SUM (sum09) / SUM (sum08) * 100 - 100)
          rost_perc,
       AVG (
          (SELECT SUM (bonus)
             FROM a13_va9_files st
            WHERE     st.tn IN
                         (SELECT slave
                                 FROM full
                                WHERE master =
       ...
       AVG (
          ...
          files_bonus_traid
  FROM (  SELECT z1.*,
                 DECODE (SIGN (rost_perc - 20), -1, 0, 1) is_act,
                   (NVL (sum09, 0) - NVL (sum08, 0))
                 * 0.1
                 * DECODE (SIGN (rost_perc - 20), -1, 0, 1)
                    bonus_plan
            FROM (SELECT z.*,
                         DECODE (
                            SIGN (
                            ...
                            DECODE (NVL (sum08, 0),
                                    ...
                                    sum09 / sum08 * 100 - 100))
                            rost_perc,
                         DECODE (SIGN (NVL (sum09, 0) - NVL (sum08, 0)),
                                 -1, 0,
                                 NVL (sum09, 0) - NVL (sum08, 0))
                            rost_uah
                    FROM (  SELECT v.tab_num,
                                   ...
                                   (SELECT parent
                                     ...
                                     WHERE tn = st.tn)
                                      parent_tn,
                                   TO_CHAR (vtps.ok_ts_date, 'dd.mm.yyyy')
                                      ok_ts_date,
                                   TO_CHAR (vtps.ok_chief_date,
