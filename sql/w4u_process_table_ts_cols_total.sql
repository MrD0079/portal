/* Formatted on 18.03.2013 15:35:41 (QP5 v5.163.1008.3004) */
  SELECT SUM (wm.akb) m_akb,
         SUM (wbm.akb) bm_akb,
         ...
         TRUNC (DECODE (SUM (NVL (wm.akb, 0)), 0, 0, SUM (wm.qtytt) / SUM (NVL (wm.akb, 0)) * 100) - DECODE (SUM (NVL (wbm.akb, 0)), 0, 0, SUM (wbm.qtytt) / SUM (NVL (wbm.akb, 0)) * 100)) * 100 fact_ball,
         v.prod_id,
         v.cat,
         vp.product_name_sw
    FROM w4u wm,
...
GROUP BY v.prod_id, v.cat, vp.product_name_sw
ORDER BY v.cat, vp.product_name_sw