/* Formatted on 12/3/2015 5:03:17  (QP5 v5.252.13127.32867) */
SELECT SUM (o_cnt) o_cnt,
  ...
  FROM (  SELECT id_net,
                 ...
                 PLAN,
                 fakt,
                 DECODE (PLAN, 0, 0, SUM (o_total) / PLAN * 100) ef_plan,
                 DECODE (fakt, 0, 0, SUM (fu_total) / fakt * 100) ef_fakt
                      ...
                      FROM (SELECT n1.*, pf1.PLAN, pf1.fakt / 1000 fakt
                           ...
                           (SELECT DISTINCT statya, id_net, payment_format
                             ...
                             WHERE     plan_type IN (3, 4)
                                   AND CASE
                                          WHEN s.parent NOT IN (42, 96882041)
                                          THEN
                                             1
                                          WHEN s.parent = 96882041
                                          THEN
                                             2
                                          WHEN s.parent = 42
                                          THEN
