         ...
         SUM ( (SELECT COUNT (*)
                 ...
                 WHERE     f.master = u.tn
                       ...
                       AND uf.res_pos_id IN (SELECT pos_id
                                              ...
                                              WHERE is_nm = 1)))
            res_nm,
         NVL (SUM (prob_prosroch.prob_ts), 0) prob_ts,
         NVL (SUM (prob_prosroch.prob_tm), 0) prob_tm,
         NVL (SUM (prob_prosroch.prob_rm), 0) prob_rm,
         NVL (SUM (prob_prosroch.prob_nm), 0) prob_nm,
         NVL (SUM (k.prosr_vac_eta), 0) prosr_vac_eta,
         NVL (SUM (k.prosr_vac_ts), 0) prosr_vac_ts,
         NVL (SUM (k.prosr_vac_tm), 0) prosr_vac_tm,
         NVL (SUM (k.prosr_vac_rm), 0) prosr_vac_rm,
         NVL (SUM (k.prosr_vac_nm), 0) prosr_vac_nm,
         DECODE (
            SUM (prob_test_perc.ts_max_ball),
            0, 0,
              SUM (prob_test_perc.ts_test_ball)
            / SUM (prob_test_perc.ts_max_ball)
            * 100)
            ts_perc,
         DECODE (
            SUM (prob_test_perc.tm_max_ball),
            0, 0,
              SUM (prob_test_perc.tm_test_ball)
            / SUM (prob_test_perc.tm_max_ball)
            * 100)
            tm_perc,
         DECODE (
            SUM (prob_test_perc.rm_max_ball),
            0, 0,
              SUM (prob_test_perc.rm_test_ball)
            / SUM (prob_test_perc.rm_max_ball)
            * 100)
            rm_perc,
         DECODE (
            SUM (prob_test_perc.nm_max_ball),
            0, 0,
              SUM (prob_test_perc.nm_test_ball)
            / SUM (prob_test_perc.nm_max_ball)
            * 100)
            nm_perc
    FROM user_list u,
