/* Formatted on 04.02.2014 8:38:38 (QP5 v5.227.12220.39724) */
  SELECT z.*, NVL (current_accepted_id, 0) current_status
    FROM (SELECT ac.id,
                 ...
                 ac.valid_no,
                 ac.valid_tn,
                 fn_getname ( ac.valid_tn) valid_fio,
                 TO_CHAR (ac.valid_lu, 'dd.mm.yyyy hh24:mi:ss') valid_lu,
                 ac.valid_text,
                 ...
                 ac_executors.pos_name executor_pos_name,
                 ac_executors.department_name executor_department_name,
            ...
            FROM ac,
                 ac_accept,
                 (SELECT ace.*, acu.pos_name, acu.department_name
                    FROM ac_executors ace, user_list acu
                   WHERE ace.tn = acu.tn) ac_executors,
                 accept_types acat,
                 ac_files f,
                 user_list u,
                 user_list u1,
   ...
   WHERE     DECODE (:status,  0, 0,  1, 1,  2, 0,  3, 0,  4, 0) =
         ...
         AND DECODE (:ac_cat, 0, 0, :ac_cat) = DECODE (:ac_cat, 0, 0, cat_id)
