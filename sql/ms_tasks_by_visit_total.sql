/* Formatted on 13/05/2016 11:09:01 (QP5 v5.252.13127.32867) */
SELECT SUM (tsk_good) tsk_good,
       SUM (tsk_bad) tsk_bad,
       SUM (tsk_total) tsk_total,
       DECODE (SUM (tsk_total), 0, 0, SUM (tsk_good) / SUM (tsk_total) * 100)
          tsk_good_perc
  FROM (  SELECT ag_id,
                 tz_oblast,
                 city,
            ...
            FROM (  SELECT ag_id,
                           ...
                           + SUM (
                                CASE
                                   WHEN msrfc_cnt > 0 AND msrfc_response_ok = 1
                                   THEN
                                      1
                                   ELSE
                                      0
                           ...
                           + SUM (
                                CASE
                                   WHEN msrfc_cnt > 0 AND msrfc_response_ok = 0
                                   THEN
                                      1
                                   ELSE
                                      0
                      ...
                      FROM (SELECT DISTINCT
                                   ...
                                   x.net_name,
                                   x.ag_name,
                                   fn_get_mc_response_ok (x.data,
                                                          x.ag_id,
                                                          x.kodtp)
                                      mc_response_ok,
                                   FN_GET_mc_cnt (x.data, x.ag_id, x.kodtp)
                                      mc_cnt,
                                   fn_get_msrfc_response_ok (x.data,
                                                             x.ag_id,
                                                             x.kodtp)
                                      msrfc_response_ok,
                                   FN_GET_msrfc_cnt1 (x.data, x.ag_id, x.kodtp)
                                      msrfc_cnt
                                     ...
                                     WHERE     (   rh_tn IN (SELECT slave
                                           ...
                                           AND DECODE ( :city,
                                                  ...
                                                  DECODE ( :city,
                                                          '0', '0',
                                                          :city)) x
                             WHERE    FN_GET_msrfc_cnt1 (x.data,
                                                         x.ag_id,
                                                         x.kodtp) > 0
                                   OR FN_GET_mc_cnt (x.data, x.ag_id, x.kodtp) >
                                         0)
                  GROUP BY ag_id,
                           tz_oblast,
                           city,
