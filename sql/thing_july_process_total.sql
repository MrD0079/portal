/* Formatted on 11.01.2013 13:47:05 (QP5 v5.163.1008.3004) */
  SELECT fio,
         COUNT (*) c,
         tab_num,
         ROUND (AVG (b1 + b2 + b3 + b4), 2) bt,
         fn_getname (
                      (SELECT parent
                         FROM parents
                        WHERE tn = z1.tn))
...
GROUP BY fio, tab_num, z1.tn
ORDER BY parent_fio, fio