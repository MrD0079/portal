/* Formatted on 30.12.2016 13:23:20 (QP5 v5.252.13127.32867) */
  SELECT rmkk,
         rmkk_name,
         nm_lu,
         nm_ok,
         SUM (tm_prev_sum_per) tm_prev_sum_per,
         SUM (tm_prev_income_sum) tm_prev_income_sum,
         SUM (sum_per) sum_per,
         SUM (income_sum) income_sum,
         SUM (total1) total1,
         SUM (fou_total) fou_total,
         SUM (faktokazusl_total) faktokazusl_total,
         SUM (fu_prev_total) fu_prev_total,
         SUM (remain) remain
    FROM (  SELECT rmkk,
                   rmkk_name,
                   mkk_ter,
              ...
              FROM (  SELECT m.rmkk,
                             ...
                             CASE WHEN n.tn_mkk <> m.mkk_ter THEN 1 ELSE 0 END
                                mkk_diff,
                             m.mkk_ter,
                       ...
                       WHERE     m.id_net = n.id_net
                             ...
                             AND (   (    m.mkk_ter IN (SELECT slave
                                                          FROM full
                                                         WHERE master = :tn)
                                      AND DECODE ( :tn_rmkk, 0, m.rmkk, :tn_rmkk) =
                                             m.rmkk
                                      AND DECODE ( :tn_mkk, 0, m.mkk_ter, :tn_mkk) =
                                             m.mkk_ter)
                                  OR (    :tn IN (DECODE (
                                                     (SELECT pos_id
                                                        FROM spdtree
