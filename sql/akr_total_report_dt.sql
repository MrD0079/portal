/* Formatted on 21.07.2014 10:23:44 (QP5 v5.227.12220.39724) */
  SELECT TO_CHAR (dt.dt, 'dd.mm.yyyy') dt,
   ...
   WHERE     dt.dt = agr.dt(+)
         AND dt.dt = avkr.dt(+)
         AND :tz_id = agr.tz_id(+)
         AND :tz_id = avkr.tz_id(+)
         AND dt.dt BETWEEN TO_DATE (:sdt, 'dd.mm.yyyy')
                       AND TO_DATE (:edt, 'dd.mm.yyyy')
ORDER BY dt.dt