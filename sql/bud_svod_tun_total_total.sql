/* Formatted on 03.08.2017 13:31:42 (QP5 v5.252.13127.32867) */
SELECT COUNT (*) c,
       SUM (summa) summa,
       ...
       SUM (total) total,
       DECODE (SUM (DECODE (selected, 1, summa, NULL)), 0, 0, NVL (SUM (DECODE (selected, 1, total, NULL)) / SUM (DECODE (selected, 1, summa, NULL)) * 100, 0)) zat,
       SUM (bonus_fakt) bonus_fakt,
       SUM (maxtp) maxtp,
       SUM (cash) cash,
       SUM (compens_distr) compens_distr,
       DECODE (SUM (summa), 0, 0, -SUM (skidka_val) / SUM (summa) * 100) skidka,
  ...
  FROM (SELECT 'tp' || s.tp_kod,
               ...
               (  SELECT tp_kod,
                    ...
                    FROM (SELECT z.id,
                           ...
                           WHERE     (SELECT NVL (tu, 0)
                                 ...
                                 AND TO_NUMBER (getZayFieldVal (z.id, 'admin_id', 4)) IS NOT NULL)
         ...
         WHERE     zp.fil = taf.fil(+)
               ...
               AND DECODE ( :ok_bonus,  1, 0,  2, sv.tp_kod) = DECODE ( :ok_bonus,  1, 0,  2, NVL (sv.tp_kod, 0))
