/* Formatted on 08/12/2014 12:45:20 (QP5 v5.227.12220.39724) */
SELECT (SELECT sales
          FROM nets_plan_year
       ...
       (SELECT sales_coffee
          FROM nets_plan_year
         WHERE YEAR = :YEAR AND plan_type = :plan_type AND id_net = :net)
          plan_cur_year_coffee,
       (SELECT sales_prev_coffee
          FROM nets_plan_year
         WHERE YEAR = :YEAR AND plan_type = :plan_type AND id_net = :net)
          plan_prev_year_coffee,
       DECODE (
          NVL (
             (SELECT sales_prev
                FROM nets_plan_year
  ...
  FROM (SELECT SUM (CASE pay_format WHEN 1 THEN summa END) b_perc,
               SUM (CASE pay_format WHEN 2 THEN summa END) b_sum
          FROM sdu_terms_year
         WHERE id_net = :net AND ver = :ver AND YEAR = :YEAR) z