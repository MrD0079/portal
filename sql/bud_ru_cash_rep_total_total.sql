/* Formatted on 16.12.2013 15:05:05 (QP5 v5.227.12220.39724) */
SELECT SUM (remain_start_plan) remain_start_plan,
       SUM (remain_start_fakt) remain_start_fakt,
       SUM (in_rep_plan) in_rep_plan,
       SUM (in_rep_fakt) in_rep_fakt,
       SUM (sum_start_plan) sum_start_plan,
       SUM (sum_start_fakt) sum_start_fakt,
       SUM (out_rep_plan) out_rep_plan,
       SUM (out_rep_fakt) out_rep_fakt,
         DECODE (SUM (sum_start_fakt),
                 0, 0,
                 SUM (out_rep_fakt) / SUM (sum_start_fakt))
       * 100
          out_perc_fakt,
       SUM (zd_fakt) zd_fakt,
       SUM (sum_end_plan) sum_end_plan,
       SUM (sum_end_fakt) sum_end_fakt,
       SUM (ok_fil) ok_fil
  FROM (  SELECT f.id,
                 f.name,
                 f.nd,
                 ...
                 * 100
                 ...
                 + SUM (
                    ...
                    sum_end_fakt,
                 CASE WHEN SUM (c_in_rep.ok_fil) = COUNT (*) THEN 1 ELSE 0 END
                    ok_fil
            FROM (SELECT f.*, h.db
                 ...
                 (  SELECT h_in.fil,
                           ...
                           SUM (b_in.plan) plan,
                           SUM (b_in.fakt) fakt,
                           CASE WHEN SUM (ok_fil) = COUNT (*) THEN 1 ELSE 0 END
                              ok_fil
                      FROM bud_ru_cash_in_head h_in, bud_ru_cash_in_body b_in
                     WHERE     h_in.id = b_in.head_id
           ...
           WHERE     f.id = c_in_prev.fil(+)
                 ...
                 AND (   F.DATA_END IS NULL
                      OR F.DATA_END >= TO_DATE (:sd, 'dd/mm/yyyy'))
        GROUP BY f.id,
                 f.name,
                 f.nd,
                 f.rp,
