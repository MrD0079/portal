/* Formatted on 29.07.2013 15:33:02 (QP5 v5.227.12220.39724) */
  SELECT a13_sp8.tab_num,
         ...
         a13_sp8.tp_kod,
         a13_sp8tps.selected,
         NVL (a13_sp8tps.contact_lpr,
              (SELECT TRIM (contact_tel || ' ' || contact_fio)
                 FROM routes
                WHERE tp_kod = a13_sp8.tp_kod AND ROWNUM = 1))
            contact_lpr,
           2
         - CASE
    ...
    FROM a13_sp8_tp_select a13_sp8tps,
         a13_sp8_action_nakl an,
         a13_sp8,
         user_list st
   WHERE     a13_sp8.tab_num = st.tab_num
         AND (   st.tn IN (SELECT slave
              ...
              OR (SELECT NVL (is_traid, 0)
                    FROM user_list
                   WHERE tn = :tn) = 1)
         AND a13_sp8.tp_kod = a13_sp8tps.tp_kod
         AND a13_sp8.H_TP_KOD_DATA_NAKL = an.H_TP_KOD_DATA_NAKL(+)
         AND st.dpt_id = :dpt_id
         AND DECODE (:eta_list, '', a13_sp8.h_fio_eta, :eta_list) =
                a13_sp8.h_fio_eta
         AND a13_sp8tps.selected = 1
         AND a13_sp8.data BETWEEN TO_DATE ('01.08.2013', 'dd.mm.yyyy')
                              AND TO_DATE ('31.08.2013', 'dd.mm.yyyy')
GROUP BY a13_sp8.tab_num,
         st.fio,
         a13_sp8.fio_eta,
         a13_sp8.tp_ur,
         a13_sp8.tp_addr,
