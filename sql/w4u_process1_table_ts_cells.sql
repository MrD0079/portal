/* Formatted on 24.07.2013 16:11:33 (QP5 v5.227.12220.39724) */
  SELECT v.tab_num,
         v.fio_eta fio,
         v.h_fio_eta tn,
         NVL (wm.ttqty, 0) m_qtytt,
         NVL (wbm.ttqtybaseperiod, 0) bm_qtytt,
         NVL (wm.akb, 0) m_akb,
         NVL (wbm.akbprev, 0) bm_akb,
         v.H_GROUPNAME prod_id,
         v.m,
         v.bm,
         v.ball,
         v.min_por,
         DECODE (NVL (wm.akb, 0), 0, 0, wm.ttqty / NVL (wm.akb, 0) * 100) perc,
         DECODE (NVL (wbm.akbprev, 0),
                 0, 0,
                 wbm.ttqtybaseperiod / NVL (wbm.akbprev, 0) * 100)
            bperc,
           CASE
...
ORDER BY v.cat, vp.groupname, v.fio_eta