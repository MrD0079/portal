/* Formatted on 06.02.2017 18:55:00 (QP5 v5.252.13127.32867) */
  SELECT rmkk,
         rmkk_name,
         tm_prev_sum_per,
         SUM (sum_per) sum_per,
         ...
         - NVL (SUM (faktoplachusl_prev_total), 0)
         + NVL (SUM (sum_per), 0)
         - NVL (SUM (faktoplachusl_cur_total), 0)
            remain,
         NVL (tm_prev_sum_per, 0) - NVL (SUM (faktoplachusl_prev_total), 0)
            remain_prev
    FROM (  SELECT y,
                   my,
                   rmkk,
          ...
          ORDER BY y,
                   ...
                   rmkk_name,
                   mkk_name)
GROUP BY rmkk, rmkk_name, tm_prev_sum_per
ORDER BY rmkk_name