/* Formatted on 02/06/2015 10:56:55 (QP5 v5.227.12220.39724) */
SELECT SUM (msb_weight) msb_weight,
       ...
       COUNT (*) c
  FROM (/* Formatted on 18/10/2016 22:56:52 (QP5 v5.252.13127.32867) */
  SELECT TO_CHAR (NVL (msr.r_DT, TO_DATE ( :ed, 'dd/mm/yyyy')), 'dd.mm.yyyy')
            dt,
 ...
 FROM ms_rep_hbr,
      (  SELECT h_id, MAX (r_dt) dt
       ...
       GROUP BY h_id) h1
WHERE ms_rep_hbr.b_head_id = h1.h_id AND ms_rep_hbr.r_dt = h1.dt)*/
        ms_rep_hbr_max_dt msr,
         MS_REP_HBR_DT msh,
         merch_spec_body msb,
         merch_report mr,
         ms_rep_routes1             /*,
   ...
   WHERE     msh.kod_tp = ms_rep_routes1.rb_kodtp
         ...
         AND DECODE ( :city, '0', '0', ms_rep_routes1.cpp1_city) =
                DECODE ( :city, '0', '0', :city) /*AND mro.dt = msr.r_dt
