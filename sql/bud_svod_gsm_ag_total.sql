/* Formatted on 09/04/2015 17:43:58 (QP5 v5.227.12220.39724) */
SELECT COUNT (*) c,
       SUM (fal_payment) fal_payment,
       SUM (probeg) probeg,
       SUM (gbo) gbo,
       SUM (gbo_warmup) gbo_warmup,
       SUM (amort) amort,
       SUM (total1) total1,
       SUM (sales) sales,
       DECODE (SUM (fal_payment),
          ...
          amort_perc,
       DECODE (SUM (sales), 0, 0, SUM (total1) / SUM (sales) * 100) perc_zat
  FROM (SELECT NVL (sv.id, FN_GET_NEW_ID) id,
          ...
          FROM (SELECT DISTINCT m.tab_num,
                 ...
                 WHERE     m.dpt_id = :dpt_id
                       AND TO_DATE (:dt, 'dd.mm.yyyy') = m.dt) r,
               user_list u,
               bud_svod_zp sv,
               bud_fil f,
               (SELECT fil, ok_db_tn
