/* Formatted on 23.01.2013 10:22:50 (QP5 v5.163.1008.3004) */
  SELECT ROWNUM,
         emp_tn,
         fio,
         pos_name,
    ...
    FROM (SELECT e.emp_tn,
                 ...
                 (SELECT SUM (s.score * c.weight) / COUNT (e1.year)
                    FROM ocenka_score s, ocenka_criteria c, emp_exp ee
                   WHERE s.criteria = c.id_num AND s.tn = e.emp_tn AND s.tn = ee.emp_tn AND s.exp_tn = ee.exp_tn AND ee.FULL = 0 AND s.event = e1.year AND c.event = e1.year AND s.exp_tn <> e.emp_tn)
                    exp_avg_score,
                 (SELECT SUM (s.score)
                    FROM ocenka_score s
