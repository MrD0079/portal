/* Formatted on 14.10.2016 13:50:57 (QP5 v5.252.13127.32867) */
  SELECT np.y,np.my,
         np.mt,
         np.fil_id,
         np.fil_name,
    ...
    FROM (SELECT n.sw_kod,
           ...
           WHERE     (   f.data_end IS NULL
                      OR f.data_end >=
                            (SELECT MIN (data)
                               FROM calendar
                              WHERE y = :y AND ( :m = 0 OR my = :m) AND dm = 1))
         ...
         (  SELECT id_net,
                   ...
                   year,
                   month,
                   SUM (CASE WHEN m.payment_type = 1 THEN total END) opz_pt_f2,
                   SUM (CASE WHEN m.payment_type = 2 THEN total END) opz_pt_f1,
                   SUM (CASE WHEN m.payment_type = 3 THEN total END) opz_pt_t,
                   SUM (CASE WHEN m.payment_type IN (1, 2, 3) THEN total END)
                      opz_total
              FROM nets_plan_month m
             WHERE plan_type = 3
          GROUP BY id_net,
         ...
         (  SELECT id_net,
                   ...
                   year,
                   month,
                   SUM (CASE WHEN m.payment_type = 1 THEN total END) fou_pt_f2,
                   SUM (CASE WHEN m.payment_type = 2 THEN total END) fou_pt_f1,
                   SUM (CASE WHEN m.payment_type = 3 THEN total END) fou_pt_t,
                   SUM (CASE WHEN m.payment_type IN (1, 2, 3) THEN total END)
                      fou_total
              FROM nets_plan_month m
             WHERE plan_type = 4
         ...
         (  SELECT m.id_net,
                   ...
                   m.year,
                   m.month,
                   SUM (CASE WHEN m.payment_type = 1 THEN id.summa END) bu_pt_f2,
                   SUM (CASE WHEN m.payment_type = 2 THEN id.summa END) bu_pt_f1,
                   SUM (CASE WHEN m.payment_type = 3 THEN id.summa END) bu_pt_t,
