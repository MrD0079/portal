/* Formatted on 07.11.2017 18:23:13 (QP5 v5.252.13127.32867) */
  SELECT fil_id,
         fil_name,
         fund_id,
         fund_name,
    ...
    FROM (  SELECT tf.fil_id,
                   ...
                   CASE
                      ...
                      THEN
                         CASE
                            WHEN (  NVL (sales.sales_fact * (tf.norm - 0.5) / 100,
                                         0)
                                  - NVL (zp.total1, 0)
                            ...
                            ELSE
                                 (  NVL (sales.sales_fact * (tf.norm - 0.5) / 100,
                                         0)
                                  - NVL (zp.total1, 0)
                                  - NVL (zay.compens_distr, 0))
                               + NVL (sales.sales_fact * 0.5 / 100, 0)
                      ...
                      WHEN tf.gsm = 2
                      THEN
                           (  NVL (sales.sales_fact * (tf.norm - 0.5) / 100, 0)
                            - NVL (zp.total1, 0)
                            - NVL (zay.compens_distr, 0))
                         + NVL (sales.sales_fact * 0.5 / 100, 0)
                      WHEN tf.gsm = 3
              ...
              FROM (  SELECT f.id fil_id,
                                                ...
                                                DECODE (
                                                   ...
                                                   (SELECT MAX (accept_order)
                                                     ...
                                                     WHERE     z_id = z.id
                                                           AND accepted = 2))) = 1
                             AND z.valid_no = 0
                             AND z.cost_assign_month BETWEEN TO_DATE ( :sd,
                                                                      'dd.mm.yyyy')
                                                         AND TO_DATE ( :ed,
                                                                      'dd.mm.yyyy')
                             AND (   :exp_list_without_ts = 0
                   ...
                   (/* Formatted on 06.12.2017 09:55:36 (QP5 v5.252.13127.32867) */
  SELECT period,
         fil,
         fund_id,
         SUM (sales) sales,
    ...
    FROM (  SELECT z.act_month period,
                   zp.fil,
                   z.fund_id,
                   SUM (s.sales) sales,
                   SUM (s.bonus) bonus,
          ...
          GROUP BY zp.fil, z.fund_id, z.act_month
          UNION
             ...
             WHERE     u.tab_num = s.ts_tab_num
                   ...
                   AND (   u.tn IN (SELECT slave
                        ...
                        OR (SELECT NVL (is_traid_kk, 0)
                             ...
                             WHERE tn = :tn) = 1)
                   AND s.db_tn = DECODE ( :db, 0, s.db_tn, :db)
          GROUP BY s.fil_kod, z.fund_id, z.act_month)
GROUP BY period, fil, fund_id) act,
                   (SELECT dt period,
                           cust_id,
                           cust_name,
