...
if ($a->getAuth())
{
	...
	else
	{
		...
		$smarty->assign('department_name', $a->getAuthData('department_name'));
		$smarty->assign('pos_name', $a->getAuthData('pos_name'));
		$smarty->assign('pos_id', $a->getAuthData('pos_id'));
		$_SESSION["pos_id"] = $a->getAuthData('pos_id');
		...
		if (!isset($_REQUEST["print"])&&!isset($_REQUEST["nohead"]))
		{
			...
			(
				(
				(strpos($login,'akr_avk_')===false)
				&&
				(strpos($login,'akr_ag_')===false)
				&&
				(strpos($login,'atd_avk_')===false)
				&&
				(strpos($login,'atd_ag_')===false)
				&&
				(strpos($login,'azl_avk_')===false)
				&&
				(strpos($login,'azl_ag_')===false)
				&&
		...
		{
			if (strpos($login,'akr_avk_')!==false)
			{
				include "akr_report.php";
			}
			else
			if (strpos($login,'akr_ag_')!==false)
			{
				include "akr_ag_report.php";
			}
			else
			if (strpos($login,'atd_avk_')!==false)
			{
				include "atd_report.php";
			}
			else
			if (strpos($login,'atd_ag_')!==false)
			{
				include "atd_ag_report.php";
			}
			else
			if (strpos($login,'azl_avk_')!==false)
			{
				include "azl_report.php";
			}
			else
			if (strpos($login,'azl_ag_')!==false)
			{
				include "azl_ag_report.php";
			}
			else
			if (strpos($login,'ms')!==false)
			{
				include "merch_report_new.php";
			}
			else
			if (strpos($login,'avk')!==false)
			{
				include "golos.php";
			}
			else
			if (strpos($login,'ag0')!==false)
			{
			}
			else
			if (strpos($login,'fil0')!==false)
			{
				include "bud_svod_zatd.php";
			}
			else
...
{
	// echo "<font color=\"#990000\">Ќеправильное им€ пользовател€ или пароль</font>";
}
