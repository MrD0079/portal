...
$sql = rtrim(file_get_contents('sql/hot_aug_process_1_nakl.sql'));
$sql=stritr($sql,$params);
//echo $sql;
...
$sql=rtrim(file_get_contents('sql/hot_aug_process_1_tp.sql'));
$sql=stritr($sql,$params);
...
$params=array(':dpt_id' => $_SESSION["dpt_id"],':tn'=>$tn,':tp'=>0,':pt'=>$_REQUEST["pt"],':eta_list' => "'".$_REQUEST["eta_list"]."'");
$sql=rtrim(file_get_contents('sql/hot_aug_process_1_nakl_total.sql'));
$sql=stritr($sql,$params);
$sql=stritr($sql,$params);
...
$smarty->display('hot_aug_process_1.html');
?>