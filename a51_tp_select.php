...
if (isset($_REQUEST["save"]))
{
	...
	if (isset($_REQUEST["selected_changed"]))
	{
		...
		foreach ($val as $key1 => $val1)
		{
			if ($val1!=null)
			{
				$keys = array('tp_kod'=>$key,'fio_eta'=>$key1);
				$values = array('selected'=>Bool2Int($val1));
...
$sql=rtrim(file_get_contents('sql/a51_tp_select.sql'));
$sql=stritr($sql,$params);
$tp = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign('tp', $tp);

$sql_total=rtrim(file_get_contents('sql/a51_tp_select_total.sql'));
...
$smarty->display('a51_tp_select.html');
?>