...
InitRequestVar("type",1); // по умолчанию считаем балл по С-В


$sql = rtrim(file_get_contents('sql/apvd12_lead_rm.sql'));
...
foreach ($drm as $k => $v)
{
	...
	$sql = rtrim(file_get_contents('sql/apvd12_lead_tm.sql'));
	...
	$sql = rtrim(file_get_contents('sql/apvd12_lead_ts.sql'));
	$p = array(':dpt_id' => $_SESSION["dpt_id"],':tn' => $v["tn"], ':type' => $_REQUEST["type"]);
	$sql=stritr($sql,$p);
	...
	$sql = rtrim(file_get_contents('sql/apvd12_lead_eta.sql'));
	$p = array(':dpt_id' => $_SESSION["dpt_id"],':tn' => $v["tn"], ':type' => $_REQUEST["type"]);
	$sql=stritr($sql,$p);
...
$smarty->assign('d', $d);
$smarty->display('apvd12_lead.html');
