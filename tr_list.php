...
if (isset($_REQUEST["save"])||isset($_REQUEST["ok_primary_retry"]))
{
	...
	if (isset($_REQUEST["del"]))
	{
		...
		{
			Table_Update("tr_order_head",array("id"=>$v),null);
	...
	if (isset($_REQUEST["ok_primary_retry"]))
	{
		if (isset($_REQUEST["ok_primary_retry_id"]))
		{
				...
				$sql=rtrim(file_get_contents('sql/tr_list_order_ok_primary_parents.sql'));
				$sql=stritr($sql,$p);
				...
				foreach ($d2 as $k1 => $v1)
				{
					...
					Продолжительность тренинга: ".$d1["days"].". <br />
	...
	if (isset($_REQUEST["ok_primary"]))
	{
		...
		{
			Table_Update("tr_order_head",array("id"=>$k),array("ok_primary"=>1));
				...
				//Тренер – $тренер,
				//Локация – $локация.
				//<a href="https://ps.avk.ua/?action=tr_list">Перейти</a> на страницу подтверждения».

				$sql=rtrim(file_get_contents('sql/tr_list_order_nbt_emails.sql'));
				$sql=stritr($sql,$p);
				$e=$db->getCol($sql);
				$emails=join($e,',');
	...
	if (isset($_REQUEST["ok_final"]))
	{
		foreach($_REQUEST["ok_final"] as $k=>$v)
		{
			if ($v!='')
			{
				Table_Update("tr_order_head",array("id"=>$k),array("ok_final"=>$v));
			}
			...
			if (($v==1)&&($d1["days2tr"]>0))
			{
				...
				$sql=rtrim(file_get_contents('sql/tr_list_order_chat.sql'));
				$sql=stritr($sql,$p);
				...
				$sql=rtrim(file_get_contents('sql/tr_list_order_emails.sql'));
				$sql=stritr($sql,$p);
				$e=$db->getCol($sql);
				$emails=join($e,',');
				...
				$sql=rtrim(file_get_contents('sql/tr_list_order_ok_final_childs.sql'));
				$sql=stritr($sql,$p);
				...
				/* Участники тренинга получают приглашение от информационной системы: */
				...
				foreach ($d2 as $k1 => $v1)
				{
					$subj="Вы приглашены для участия в тренинге - ".$d1["name"].".";
					$text="Здравствуйте, ".$v1["c_fio"].".<br /><br />
					Вы приглашены для участия в тренинге - ".$d1["name"].". <br />
					Тренер: ".$d1["fio"].". <br />
					Дата проведения тренинга: ".$d1["dt_start_t"].". <br />
					Место проведения тренинга: ".$d1["loc_name"].". <br />".
					//"<br />".
					//"Начало тренинга: ".$parameters['tr_time_start']['val_string']." (по местному времени).".
					//"Окончание тренинга: ".$d1["tr_end"].", ".$parameters['tr_time_end']['val_string']." (по местному времени). <br />".
					"<br />".
					"Ссылка на карту места проведения тренинга: <a href=\"".$d1["url"]."\">".$d1["url"]."</a><br>
					<br />
					".nl2br($d1["loc_text"])."<br>
					<br />
					С собой Вам необходимо иметь средства, достаточные для оплаты гостиницы и питания.</br>
					Данные затраты будут Вам компенсированы.</br>
					По всем дополнительным вопросам Вы можете обращаться напрямую к тренеру, проводящему данный тренинг.<br>";
					if ($d1["days"]>=2)
					{
						$text.="Для всех сотрудников, основное место работы которых географически отдалено более чем на 100 км от места проведения тренинга, будет забронирована гостиница.<br>
						Условия бронирования: ".$d1["nights"]." ночевка(-и), с ".$d1["dt_start_t"]."по ".$d1["tr_end"].".<br>
						Если по каким-либо причинам (есть где жить; более ранний приезд / более поздний отъезд с места проведения тренинга) вам нужны другие условия бронирования гостиницы,<br>
						Вам необходимо в течение 24 часов с момента получения данного сообщения связаться с тренером и сообщить об этом.<br>";
					}
					$text.="<br>";
					/*if (count($dc)>0)
					{
						$text.="Дополнительные комментарии:<br>";
						$text.=$chat;
					}*/
	...
	if (isset($_REQUEST["chat"]))
	{
		foreach($_REQUEST["chat"] as $k=>$v)
		{
			if ($v!='')
			{
				...
				Table_Update("tr_order_chat",$keys,$keys);
...
{
	$sql=rtrim(file_get_contents('sql/tr_list.sql'));
	$sql=stritr($sql,$p);
	//echo $sql;
	$tr_list = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
	//var_dump($tr_list);
	foreach($tr_list as $k=>$v)
	{
		$d[$v["id"]]["head"]=$v;
