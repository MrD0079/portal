...
$sql=rtrim(file_get_contents('sql/tr_pt_os_test_result.sql'));
$sql=stritr($sql,$p);
...
foreach ($tru as $k => $v)
{
	...
	$sql=rtrim(file_get_contents('sql/tr_pt_os_test_result_tests.sql'));
	$sql=stritr($sql,$p);
	...
	$sql=rtrim(file_get_contents('sql/tr_pt_os_test_result_test_res.sql'));
	$sql=stritr($sql,$p);
...
$smarty->display('tr_pt_os_test_result.html');
