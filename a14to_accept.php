...
else
{
	...
	{
		$sql=rtrim(file_get_contents('sql/a14to_accept.sql'));
		$sql=stritr($sql,$params);
		//echo $sql;
		$d = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
		foreach($d as $k=>$v)
		{
			$sql=rtrim(file_get_contents('sql/a14to_accept_photos.sql'));
		...
		$smarty->assign('d', $d);
		$sql=rtrim(file_get_contents('sql/a14to_accept_total.sql'));
		$sql=stritr($sql,$params);
		$t = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
		$smarty->assign('t', $t);
		$sql=rtrim(file_get_contents('sql/a14to_accept_total1.sql'));
		$sql=stritr($sql,$params);
		$t1 = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
		$smarty->assign('t1', $t1);
	}
	$smarty->display('a14to_accept.html');
}
?>