		...
		if (isset($_FILES["files"]))
		{
			...
			foreach ($z['tmp_name'] as $k=>$v)
			{
				if (is_uploaded_file($z["tmp_name"][$k]))
				{
					...
					move_uploaded_file($z["tmp_name"][$k], "merch_spec_report_files_chat_files/".$fn);
					$image = new SimpleImage();
					$image->load("merch_spec_report_files_chat_files/".$fn);
					$h=$image->getHeight();
					if ($image->getHeight()>600)
					{
						$image->resizeToHeight(600);
					}
					$image->save("merch_spec_report_files_chat_files/".$fn);
...
else
{
	...
	$sql = rtrim(file_get_contents('sql/merch_report_4sv_by_spec_new_file_chat.sql'));
	$sql=stritr($sql,$p);
	$res = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
	$smarty->assign('chat', $res);
	$sql = rtrim(file_get_contents('sql/merch_report_4sv_by_spec_new_file_chat_f.sql'));
	$sql=stritr($sql,$p);
	$res = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
	$smarty->assign('chat_f', $res);
	$smarty->display('merch_report_4sv_by_spec_new_file_chat.html');
}
?>