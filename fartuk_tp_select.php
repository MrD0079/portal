...
if (isset($_REQUEST["save"]))
{
	...
	if (isset($_REQUEST["selected_changed"]))
	{
		foreach ($_REQUEST["selected_changed"] as $key => $val)
		{
			if ($val!=null)
			{
				$keys = array('h_custcode_kodtt'=>$key);
...
$sql=rtrim(file_get_contents('sql/fartuk_tp_select.sql'));
...
$sql_total=rtrim(file_get_contents('sql/fartuk_tp_select_total.sql'));
...
$smarty->display('fartuk_tp_select.html');
?>