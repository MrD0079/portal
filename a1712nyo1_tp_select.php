...
if (isset($_REQUEST["save"]))
{
    if (isset($_REQUEST["data"]))
    {
        foreach ($_REQUEST["data"] as $key => $val)
        {
            ...
            $sql="
                SELECT COUNT (*)
                  FROM (select tp_kod FROM a1712nyo2_select
                        union
            ...
            {
                Table_Update ("a1712nyo1_select", $keys, $val);
            }
        }
    }
    if (isset($_REQUEST["del"]))
    {
        ...
        {
            Table_Update ("a1712nyo1_select", array('tp_kod'=>$key), null);
        }
    }
}
$params=array(':dpt_id' => $_SESSION["dpt_id"],':tn'=>$tn);
$sql=rtrim(file_get_contents('sql/a1712nyo1_tp_select.sql'));
$sql=stritr($sql,$params);
$tp = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign('tp', $tp);
$sql=rtrim(file_get_contents('sql/a1712nyo1_tp_select_total.sql'));
$sql=stritr($sql,$params);
$tp = $db->getOne($sql);
$smarty->assign('tp_total', $tp);
$smarty->display('a1712nyo1_tp_select.html');
?>