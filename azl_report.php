...
else
{
	...
	$sql=rtrim(file_get_contents('sql/azl_report_tz.sql'));
	$p = array(":id" => $r['id']);
	$sql=stritr($sql,$p);
	...
	if ($_REQUEST['tz']!=0)
	{
		if (isset($_REQUEST['save']))
		{
			...
			$vals = $_REQUEST['head'];
			Table_Update('azl_tz_report',$keys,$vals);
		}
		
		$sql=rtrim(file_get_contents('sql/azl_report_get_tz.sql'));
		...
		$sql=rtrim(file_get_contents('sql/azl_report_head.sql'));
		...
		if (isset($z['id']))
		{
			if (isset($_REQUEST['save']))
			{
				...
				foreach ($_REQUEST['detail'] as $k=>$v)
				{
					...
					$vals = array('val'=>$v);
					Table_Update('azl_tz_report_params',$keys,$vals);
				}
			}
			$rep_id = $z['id'];
		...
		$sql=rtrim(file_get_contents('sql/azl_report_detail.sql'));
		$p = array(":id" => $rep_id);
		$sql=stritr($sql,$p);
		//echo $sql;
		$z1 = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
		//print_r($z1);
		$smarty->assign('azl_detail', $z1);



		$sql=rtrim(file_get_contents('sql/azl_report_files.sql'));
		$p = array(":tz_id" => $_REQUEST['tz'],':dt'=>"'".$_REQUEST['dt']."'");
		$sql=stritr($sql,$p);
	...
	$smarty->display('azl_report.html');
}
