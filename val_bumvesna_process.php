...
$table_name = "val_bumvesna_action_nakl";
...
{
$fn=$db->getOne("select fn from val_bumvesna_files where id=".$k);
unlink("val_bumvesna_files/".$tn."/".$fn);
$fn=$db->query("delete from val_bumvesna_files where id=".$k);
}
}
...
if (isset($_FILES["new_fn"]))
{
	if ($_FILES["new_fn"]["name"]!="")
	{
		...
		if ($_FILES["new_fn"]['error']==0){
			Table_Update("val_bumvesna_files",$_REQUEST["new"],$_REQUEST["new"]);
		}
	}
}

$params=array(':dpt_id' => $_SESSION["dpt_id"],':tn'=>$tn,':tp_kat'=>1,':exp_list_without_ts' => 0,':exp_list_only_ts' => 0);
$sql=rtrim(file_get_contents('sql/val_bumvesna_files.sql'));
...
$sql = rtrim(file_get_contents('sql/val_bumvesna_process_eta_list.sql'));
$sql=stritr($sql,$params);
...
$sql = rtrim(file_get_contents('sql/val_bumvesna_process_nakl.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/val_bumvesna_process_tp.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/val_bumvesna_process_nakl_total.sql'));
$sql=stritr($sql,$params);
...
$smarty->display('val_bumvesna_process.html');
?>