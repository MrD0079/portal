...
$sql = rtrim(file_get_contents('sql/a13_sv9_report_eta_list.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_sv9_report_tp.sql'));
$sql=stritr($sql,$params);
...
$sql = rtrim(file_get_contents('sql/a13_sv9_report_nakl.sql'));
$sql=stritr($sql,$params);
//echo $sql;
...
{
$fn=$db->getOne("select fn from a13_sv9_files where id=".$k);
unlink("a13_sv9_files/".$tn."/".$fn);
$fn=$db->query("delete from a13_sv9_files where id=".$k);
}
}
...
if (isset($_FILES["new_fn"]))
{
	if ($_FILES["new_fn"]["name"]!="")
	{
		...
		if ($_FILES["new_fn"]['error']==0){
			Table_Update("a13_sv9_files",$_REQUEST["new"],$_REQUEST["new"]);
		}
	}
}
...
if (isset($_REQUEST["save"]))
{
	if (isset($_REQUEST["data"]))
	{
		foreach($_REQUEST["data"] as $k=>$v)
		{
			...
			//print_r($keys);
			//print_r($v);
			Table_Update ('a13_sv9_action_nakl', $keys, $v);
		}
	}
	if (isset($_REQUEST["data_files"]))
	{
		...
		{
			$keys = array('id'=>$k);
			Table_Update ('a13_sv9_files', $keys, $v);
		}
	}
}

$sql=rtrim(file_get_contents("sql/a13_sv9_report.sql"));
$sql=stritr($sql,$params);
$list = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign("list", $list);

$sql=rtrim(file_get_contents("sql/a13_sv9_report_itogi.sql"));
$sql=stritr($sql,$params);
//echo $sql;
$list = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign("itogi", $list);

$sql=rtrim(file_get_contents("sql/a13_sv9_report_itogi_files.sql"));
$sql=stritr($sql,$params);
$list = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
...
$sql=rtrim(file_get_contents('sql/a13_sv9_files.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_sv9_files_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
...
$sql=rtrim(file_get_contents('sql/a13_sv9_files_total_total.sql'));
$sql=stritr($sql,$params);
...
$smarty->display('a13_sv9_report.html');
