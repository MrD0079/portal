...
if (isset($_REQUEST["save"]))
{
	...
	Table_Update('standBtp', $keys,$vals);
}
else if (isset($_REQUEST["saveurl"]))
{
	...
	Table_Update('standBurl', $keys,$vals);
}
else
{
	...
	{
		$sql=rtrim(file_get_contents('sql/standBra.sql'));
		$sql=stritr($sql,$params);
		//echo $sql;
		...
		foreach($d as $k=>$v)
		{
			$sql=rtrim(file_get_contents('sql/standBra_photos.sql'));
