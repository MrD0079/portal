...
$sql = rtrim(file_get_contents('sql/a13_nv_report_eta_list.sql'));
$sql=stritr($sql,$params);
...
{
$fn=$db->getOne("select fn from a13_nv_files where id=".$k);
unlink("a13_nv_files/".$tn."/".$fn);
$fn=$db->query("delete from a13_nv_files where id=".$k);
}
}
...
if (isset($_FILES["new_fn"]))
{
	if ($_FILES["new_fn"]["name"]!="")
	{
		...
		if ($_FILES["new_fn"]['error']==0){
			Table_Update("a13_nv_files",$_REQUEST["new"],$_REQUEST["new"]);
		}
	}
}
...
//			print_r($v);
			...
			Table_Update ('a13_nv_tp_select', $keys, $v);
		}
	}
	if (isset($_REQUEST["data_files"]))
	{
		...
		{
			$keys = array('id'=>$k);
			Table_Update ('a13_nv_files', $keys, $v);
		}
	}
	if (isset($_REQUEST["del"]))
	{
		foreach ($_REQUEST["del"] as $key => $val)
		{
			...
			Table_Update ('a13_nv_tp_select', $keys, null);
		}
	}
}
...
$sql=rtrim(file_get_contents("sql/a13_nv_report.sql"));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents("sql/a13_nv_report_itogi.sql"));
$sql=stritr($sql,$params);
$list = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign("itogi", $list);

$sql=rtrim(file_get_contents("sql/a13_nv_report_itogi_files.sql"));
$sql=stritr($sql,$params);
$list = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
...
$sql=rtrim(file_get_contents('sql/a13_nv_files.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_nv_files_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
...
$sql=rtrim(file_get_contents('sql/a13_nv_files_total_total.sql'));
$sql=stritr($sql,$params);
...
$smarty->display('a13_nv_report.html');
