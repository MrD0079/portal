...
{
	$table_name = "may_action_nakl";
	audit("удалил акционную накладную","may");
...
{
$fn=$db->getOne("select fn from may_files where id=".$k);
$tip=$db->getOne("select tip from may_files where id=".$k);
unlink("may_files/".$tn."/".$tip."/".$fn);
$res=$db->query("delete from may_files where id=".$k);
audit("del file may_files/".$tn."/".$tip."/".$fn,"may");
}
}
...
if ($v!="")
{
	...
	if ($_FILES["new_fn"]["error"][$k]==0){Table_Update("may_files",$_REQUEST["new"],$_REQUEST["new"]);}
...
$sql=rtrim(file_get_contents('sql/may_process.sql'));
$sql=stritr($sql,$params);
$tp = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign('tp', $tp);

$params=array(':tn'=>$tn,':dpt_id' => $_SESSION["dpt_id"]);
$sql=rtrim(file_get_contents('sql/may_process_nakl.sql'));
$sql=stritr($sql,$params);
$nakl_list_all = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign('nakl_list_all', $nakl_list_all);

$params=array(':tn'=>$tn,':dpt_id' => $_SESSION["dpt_id"]);
$sql=rtrim(file_get_contents('sql/may_process_nakl_total.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/may_types.sql'));
$r = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign('types', $r);
...
$smarty->display('may_process.html');
?>