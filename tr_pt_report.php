...
if (isset($_REQUEST["a"]))
{
	foreach ($_REQUEST["a"] as $k => $v)
	{
		foreach ($v as $k1 => $v1)
		{
			Table_Update("tr_pt_order_body",array("id" => $k),$v);
		}
	}
}
...
{
	Table_Update("tr_pt_order_head",array("id" => $_REQUEST["id"]),array("completed" => $_REQUEST["tr"]));
	...
	if ($_REQUEST["tr"]==1)
	{
		...
		$sql=rtrim(file_get_contents('sql/tr_pt_report_parents.sql'));
		$sql=stritr($sql,$p);
		...
		foreach ($d2 as $k1 => $v1)
		{
			...
			$text.="Вашему подчиненному ".$v1["c_fio"]." необходимо заполнить ОС по пройденному тренингу и пройти посттренинговое тестирование.<br>
			Для этого ему необходимо зайти через корпоративный портал в раздел «Тестирование / ОС по тренингу и тестирование».<br>
			Тестирование необходимо пройти в течение 48 часов с момента получения данного сообщения!";
			...
			Для этого пройдите по данной <a href=\"https://ps.avk.ua/?action=tr_pt_os\">ссылке</a>, либо зайдите через корпоративный портал в раздел «Команда / Обучение / ОС по тренингу и тестирование».<br /><br />
		...
		$sql=rtrim(file_get_contents('sql/tr_pt_report_pers2.sql'));
		$sql=stritr($sql,$p);
		...
		$sql=rtrim(file_get_contents('sql/tr_pt_report_accept12.sql'));
		$sql=stritr($sql,$p);
		...
		Table_Update("tr_pt_order_body",array("head" => $_REQUEST["id"],"completed" => 1,"test" => 0),array("test" => 1));
	}
}
...
if (isset($_REQUEST["b"]))
{
	...
	{
		Table_Update("tr_pt_order_body",array("id" => $k),array("test" => 1));
	}
}
...
$sql=rtrim(file_get_contents('sql/tr_pt_report.sql'));
$sql=stritr($sql,$p);
$tru = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign('tru', $tru);

$smarty->display('tr_pt_report.html');
