...
$sql = rtrim(file_get_contents('sql/rep_spddnkf_body.sql'));
$sql=stritr($sql,$p);
$r = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$sqlu = rtrim(file_get_contents('sql/rep_spddnkf_body_ag.sql'));
foreach ($r as $k=>$v)
{
...
//print_r($r);
$smarty->assign('spd_fio', $db->getOne('select fio from rep_spddnkf_list where tn='.$_REQUEST['tn']));
$smarty->assign('period', $db->getOne('SELECT mt || \' \' || y FROM calendar WHERE data = TO_DATE (\''.$_REQUEST['month_list'].'\', \'dd.mm.yyyy\')'));
$smarty->display('rep_spddnkf_body.html');
?>