...
$sql = rtrim(file_get_contents('sql/a13_sn_report_eta_list.sql'));
$sql=stritr($sql,$params);
...
if (isset($_REQUEST["add"]))
{
	if (isset($_REQUEST["keys"]))
	{
		foreach ($_REQUEST["keys"] as $key => $val)
		{
			...
			if ($vals["if1"]!=0)
			{
				Table_Update ("a13_sn_action_nakl", $keys, $vals);
				/*$sql="
                                      SELECT COUNT (*)
                                        FROM a13_sn_action_nakl
			...
			{
				Table_Update ("a13_sn_action_nakl", $keys, null);
			}
		}
	}
}
...
{
$fn=$db->getOne("select fn from a13_sn_files where id=".$k);
unlink("a13_sn_files/".$tn."/".$fn);
$fn=$db->query("delete from a13_sn_files where id=".$k);
}
}
...
if (isset($_FILES["new_fn"]))
{
	if ($_FILES["new_fn"]["name"]!="")
	{
		...
		if ($_FILES["new_fn"]['error']==0){
			Table_Update("a13_sn_files",$_REQUEST["new"],$_REQUEST["new"]);
		}
	}
}
...
//			print_r($v);
			...
			//Table_Update ('a13_sn_action_nakl', $keys, $v);
			$sql="
			...
			isset($v["bonus_sum1"])?$x=$v["bonus_sum1"]:$x=0;
			if (($c+$x)<=345)
			{
				Table_Update ("a13_sn_action_nakl", $keys, $v);
			}
	...
	if (isset($_REQUEST["data_files"]))
	{
		...
		{
			$keys = array('id'=>$k);
			Table_Update ('a13_sn_files', $keys, $v);
		}
	}
...
$sql=rtrim(file_get_contents('sql/a13_sn_report_tp.sql'));
$sql=stritr($sql,$params);
...
$sql = rtrim(file_get_contents('sql/a13_sn_report_nakl.sql'));
$sql=stritr($sql,$params);
//echo $sql;
...
{
$sql=rtrim(file_get_contents("sql/a13_sn_report.sql"));
$sql=stritr($sql,$params);
$list = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign("list", $list);

$sql=rtrim(file_get_contents("sql/a13_sn_report_itogi.sql"));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents("sql/a13_sn_report_itogi_files.sql"));
$sql=stritr($sql,$params);
$list = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
...
$sql=rtrim(file_get_contents('sql/a13_sn_files.sql'));
$sql=stritr($sql,$params);
...
$sql=rtrim(file_get_contents('sql/a13_sn_files_total.sql'));
$sql=stritr($sql,$params);
$d = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
...
$sql=rtrim(file_get_contents('sql/a13_sn_files_total_total.sql'));
$sql=stritr($sql,$params);
...
$smarty->display('a13_sn_report.html');
