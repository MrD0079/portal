	...
	if (isset($_REQUEST["keys"]))
	{
		foreach ($_REQUEST["keys"] as $key => $val)
		{
			...
			if ($vals["if1"]!=0)
			{
				$sql="
                                        SELECT COUNT (*)
                                          FROM (select h_tp_kod_data_nakl FROM a1505vv_action_nakl
                                                union
                                                select h_tp_kod_data_nakl FROM a1505si_action_nakl)
                                         WHERE h_tp_kod_data_nakl = '".$key."'";
				$c=$db->getOne($sql);
				if ($c>0)
				{
					echo "<p>Данная накладная уже участвует в одной из акций - Вафельная весенняя, Сладкие истории!!!</p>";
				}
				else
				{
