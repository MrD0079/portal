...
if (isset($_REQUEST["save"]))
{
	...
	if (isset($_REQUEST["tp_kat"]))
	{
		foreach ($_REQUEST["tp_kat"] as $key => $val)
		{
				$keys = array('tp_kod'=>$key);
				$values = array('tp_kat'=>$val);
			Table_Update ($table_name, $keys, $values);
		}
	}
}

$sql=rtrim(file_get_contents('sql/val_mart_kat.sql'));
$sql=stritr($sql,$params);
$val_mart_kat = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign('val_mart_kat', $val_mart_kat);

$params=array(':dpt_id' => $_SESSION["dpt_id"],':tn'=>$tn);
$sql=rtrim(file_get_contents('sql/val_mart_tp_select.sql'));
$sql=stritr($sql,$params);
$tp = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign('tp', $tp);

$sql_total=rtrim(file_get_contents('sql/val_mart_tp_select_total.sql'));
...
$smarty->display('val_mart_tp_select.html');
?>