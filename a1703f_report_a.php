...
if (isset($_REQUEST["add"]))
{
	if (isset($_REQUEST["keys"]))
	{
		foreach ($_REQUEST["keys"] as $key => $val)
		{
			...
			if ($vals["if1"]!=0)
			{
				Table_Update ("a1703f_action_nakl", $keys, $vals);
			}
			else
			{
				Table_Update ("a1703f_action_nakl", $keys, null);
			}
		}
	}
}
	...
	if (isset($_REQUEST["data"]))
	{
		foreach($_REQUEST["data"] as $k=>$v)
		{
			...
			else
			{
				unset($v["max_bonus"]);
				unset($v["nakl"]);
				Table_Update ('a1703f_action_nakl', $keys, $v);
			}
		}
	}
...
$sql=rtrim(file_get_contents('sql/a1703f_report_a_tp.sql'));
...
{
$sql = rtrim(file_get_contents('sql/a1703f_report_a_nakl.sql'));
$sql=stritr($sql,$params);
