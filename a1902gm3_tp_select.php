...
if (isset($_REQUEST["save"]))
{
        ...
        foreach ($_REQUEST["data"] as $key => $val)
        {
            ...
            $sql = 'SELECT count(*)  cnt FROM (
                      ...
                      WHERE gm1.tp_kod = :tp_kod
                        UNION
                      SELECT COUNT(*) cnt
                      FROM PERSIK.a1902gm2_select gm2
                      WHERE gm2.tp_kod = :tp_kod
                    ) WHERE cnt = 1';
            ...
            if($is_already_added == 0){
                ...
                Table_Update ("a1902gm3_SELECT", $keys, $val);
            }else{
    ...
    if (isset($_REQUEST["del"]))
    {
        ...
        {
            Table_Update ("a1902gm3_SELECT", array('tp_kod'=>$key), null);
        }
    }
}
$params=array(':dpt_id' => $_SESSION["dpt_id"],':tn'=>$tn);
$sql=rtrim(file_get_contents('sql/a1902gm3_tp_select.sql'));
$sql=stritr($sql,$params);
$tp = $db->getAll($sql, null, null, null, MDB2_FETCHMODE_ASSOC);
$smarty->assign('tp', $tp);
$sql_total=rtrim(file_get_contents('sql/a1902gm3_tp_select_total.sql'));
$sql_total=stritr($sql_total,$params);
$tp_total = $db->getOne($sql_total);
$smarty->assign('tp_total', $tp_total);
$smarty->display('a1902gm3_tp_select.html');
?>