...
if (isset($_REQUEST["test_end"]))
{
	...
	if (isset($_REQUEST["answer"]))
	{
		foreach ($_REQUEST["answer"] as $k=>$v)
		{
			...
			$sql=rtrim(file_get_contents('sql/tr_test_process_get_q_res.sql'));
			$sql=stritr($sql,$p);
			$r = $db->getOne($sql);
			$r==1 ? $ball++ : null;
			$keys=array(
			...
			Table_Update('tr_order_test_res',$keys,$vals);
		}
	}
	...
	$p = array();
	$p[':id'] = $_REQUEST["head"];
	$sql=rtrim(file_get_contents('sql/tr_test_process_get_q_cnt.sql'));
	$sql=stritr($sql,$p);
...
if ($d)
{
	...
	$sql=rtrim(file_get_contents('sql/tr_test_process.sql'));
	$sql=stritr($sql,$p);
