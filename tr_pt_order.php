...
{
	echo "<h3 style='color:red'>Заявка не может быть создана, т.к. список участников пуст</h3>";
}
...
if (isset($_REQUEST["SendNBT"])&&isset($_REQUEST["table"]))
{
	...
	if ($r->numRows()==0)
	{
		...
		echo "<h3 style='color:red'>Ваша заявка добавлена и отправлена на рассмотрение. Просмотреть и отредактировать ее Вы можете на форме «Редактирование заявок на проведение тренинга».</h3>";

		$sql=rtrim(file_get_contents('sql/tr_pt_order_exist.sql'));
		$sql=stritr($sql,$p);
		$d = $db->getRow($sql, null, null, null, MDB2_FETCHMODE_ASSOC);

		$sql=rtrim(file_get_contents('sql/tr_pt_order_emails.sql'));
		$sql=stritr($sql,$p);
		$e=$db->getCol($sql);
		$emails=join($e,',');
...
$smarty->display('tr_pt_order.html');
